...
class ProcessorService:
            ...
            if (img is not None): 
                for kp in img.items(): 
                    if (not kp[0] in ProcessorService.__m_images): 
                        ProcessorService.__m_images[kp[0]] = ImageWrapper._new3611(kp[0], kp[1])
    ...
    def __reorder_cartridges() -> None:
        ...
        while k < len(ProcessorService.__m_analizer_instances): 
            ...
            first_pass4304 = True
            while True:
                if first_pass4304: first_pass4304 = False
                else: i += 1
    ...
    def get_image_by_id(image_id : str) -> 'ImageWrapper':
        ...
        if (image_id is not None): 
            res = None
            wrapres3612 = RefOutArgWrapper(None)
            inoutres3613 = Utils.tryGetValue(ProcessorService.__m_images, image_id, wrapres3612)
            res = wrapres3612.value
            if (inoutres3613): 
                return res
        if (ProcessorService.__m_unknown_image is None): 
            ProcessorService.__m_unknown_image = ImageWrapper._new3611("unknown", PullentiNerCoreInternalResourceHelper.get_bytes("unknown.png"))
    ...
    def add_image(image_id : str, content : bytearray) -> None:
        ...
        if (image_id is None): 
            return
        wr = ImageWrapper._new3611(image_id, content)
