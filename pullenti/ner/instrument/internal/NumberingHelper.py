...
class NumberingHelper:
    ...
    def calc_delta(prev : 'InstrToken1', next0_ : 'InstrToken1', can_sub_numbers : bool) -> int:
        ...
        if ((len(prev.numbers) + 1) == len(next0_.numbers) and len(next0_.numbers) > 0): 
            if (prev.typ_container_rank > 0 and prev.typ_container_rank == next0_.typ_container_rank): 
                ...
                pass
            elif (prev.num_typ == NumberTypes.ROMAN and next0_.num_typ == NumberTypes.ROMAN): 
                pass
            else: 
                return 0
            i = 0
        ...
        if ((len(prev.numbers) - 1) == len(next0_.numbers) and len(prev.numbers) > 1): 
            ...
            while i < (len(prev.numbers) - 2): 
                if (prev.numbers[i] != next0_.numbers[i]): 
                    return 0
                i += 1
            wrapn12165 = RefOutArgWrapper(0)
            inoutres2166 = Utils.tryParseInt(prev.numbers[len(prev.numbers) - 2], wrapn12165)
            n1 = wrapn12165.value
            if (not inoutres2166): 
        ...
        if ((len(prev.numbers) - 2) == len(next0_.numbers) and len(prev.numbers) > 2): 
            ...
            while i < (len(prev.numbers) - 3): 
                if (prev.numbers[i] != next0_.numbers[i]): 
                    return 0
                i += 1
            wrapn12167 = RefOutArgWrapper(0)
            inoutres2168 = Utils.tryParseInt(prev.numbers[len(prev.numbers) - 3], wrapn12167)
            n1 = wrapn12167.value
            if (not inoutres2168): 
                return 0
            if ((n1 + 1) != n2): 
        ...
        if ((len(prev.numbers) - 3) == len(next0_.numbers) and len(prev.numbers) > 3): 
            ...
            while i < (len(prev.numbers) - 4): 
                if (prev.numbers[i] != next0_.numbers[i]): 
                    return 0
                i += 1
            wrapn12169 = RefOutArgWrapper(0)
            inoutres2170 = Utils.tryParseInt(prev.numbers[len(prev.numbers) - 4], wrapn12169)
            n1 = wrapn12169.value
            if (not inoutres2170): 
                return 0
            if ((n1 + 1) != n2): 
    ...
    def extract_main_sequence(lines : typing.List['InstrToken1'], check_spec_texts : bool, can_sub_numbers : bool) -> typing.List['InstrToken1']:
        ...
        while True:
            ...
            if (li.last_number == 901): 
                pass
            if (notice_regime): 
                if (li.typ != InstrToken1.Types.NOTICE): 
                    continue
            else: 
                if (li.num_typ == NumberTypes.LETTER): 
                    pass
                if (li.typ != InstrToken1.Types.LINE): 
                    if (i == 0 and li.typ == InstrToken1.Types.NOTICE): 
                        notice_regime = True
                    else: 
                        continue
            if (res is None): 
                ...
                if (res[0].num_suffix is not None): 
                    if (li.num_suffix is not None and li.num_suffix != res[0].num_suffix): 
                        continue
                if (res[0].num_typ == NumberTypes.ROMAN): 
                    can_sub_numbers = True
        ...
        if (len(res) >= 2): 
            i0 = 0
            while i0 < len(lines): 
                if (lines[i0].typ != InstrToken1.Types.COMMENT and lines[i0].typ != InstrToken1.Types.EDITIONS): 
                    break
                i0 += 1
            if (res[0] != lines[i0]): 
                tot = res[0].begin_token.begin_char - lines[i0].begin_token.begin_char
    ...
    def create_number(owner : 'FragToken', itok : 'InstrToken1') -> None:
        if ((itok.num_begin_token is None or itok.num_end_token is None or itok.numbers is None) or len(itok.numbers) == 0): 
            return
        num = FragToken._new2171(itok.num_begin_token, itok.num_end_token, InstrumentKind.NUMBER, True, itok)
        owner.children.append(num)
        owner.number = itok.first_number
        if (len(itok.numbers) > 1): 
            owner.sub_number = PartToken.get_number(itok.numbers[1])
            if (len(itok.numbers) > 2): 
                owner.sub_number2 = PartToken.get_number(itok.numbers[2])
                if (len(itok.numbers) > 3): 
                    owner.sub_number3 = PartToken.get_number(itok.numbers[3])
    ...
    def __parse_number(t : 'Token', res : 'InstrToken1', prev : 'InstrToken1') -> None:
        if (((isinstance(t, NumberToken)) and t.int_value is not None and t.typ == NumberSpellingType.DIGIT) and (t.int_value < 3000)): 
            ...
            if (nwp is not None): 
                if (nwp.end_token.is_whitespace_before and nwp.end_token.chars.is_capital_upper): 
                    pass
                else: 
                    return
            ...
            while True:
                ...
                if ((ttt.is_char_of("._") and not ttt.is_whitespace_after and (isinstance(ttt.next0_, NumberToken))) and ((ttt.next0_.typ == NumberSpellingType.DIGIT or (((ttt.next0_.typ == NumberSpellingType.WORDS)) and ttt.next0_.chars.is_latin_letter and not ttt.is_whitespace_after)))): 
                    ok1 = True
                elif (ttt.is_char_of("(<") and (isinstance(ttt.next0_, NumberToken)) and ttt.next0_.next0_ is not None): 
                    if (ttt.next0_.next0_.is_char_of(")>")): 
                        ok2 = True
                    elif ((ttt.next0_.next0_.is_char('.') and (isinstance(ttt.next0_.next0_.next0_, NumberToken)) and ttt.next0_.next0_.next0_.next0_ is not None) and ttt.next0_.next0_.next0_.next0_.is_char_of(")>")): 
                        ok3 = True
                ...
                if (((isinstance(ttt, TextToken)) and ttt.length_char == 1 and ttt.chars.is_letter) and not ttt.is_whitespace_before and len(res.numbers) == 1): 
                    ...
                    t = res.end_token
                    continue
                if ((((ttt.is_hiphen and len(res.numbers) > 1 and (isinstance(ttt.previous, NumberToken))) and not ttt.is_whitespace_after and not ttt.is_whitespace_before) and (isinstance(ttt.next0_, NumberToken)) and ttt.next0_.next0_ is not None) and ttt.next0_.next0_.is_char('.') and not (isinstance(ttt.next0_.next0_.next0_, NumberToken))): 
                    res.numbers.append(str(ttt.next0_.value))
                    res.num_end_token = ttt.next0_.next0_
                    res.end_token = res.num_end_token
                    t = res.end_token
        ...
        if (nt is not None and nt.typ == NumberSpellingType.ROMAN): 
            ...
            res.end_token = res.num_end_token
            t = res.end_token
            if (res.num_typ == NumberTypes.ROMAN and (((not res.num_end_token.is_whitespace_after or res.typ == InstrToken1.Types.CHAPTER or res.typ == InstrToken1.Types.SECTION) or res.typ == InstrToken1.Types.LINE))): 
                if ((t.next0_ is not None and t.next0_.is_char_of("._<(") and (isinstance(t.next0_.next0_, NumberToken))) and t.next0_.next0_.typ == NumberSpellingType.DIGIT): 
                    ...
                    if ((t.next0_ is not None and t.next0_.is_char_of("._<(") and (isinstance(t.next0_.next0_, NumberToken))) and t.next0_.next0_.typ == NumberSpellingType.DIGIT): 
                        ...
                        res.end_token = res.num_end_token
                    ttt = t.next0_
                    if (((((ttt is not None and ttt.is_hiphen and len(res.numbers) > 1) and (isinstance(ttt.previous, NumberToken)) and not ttt.is_whitespace_after) and not ttt.is_whitespace_before and (isinstance(ttt.next0_, NumberToken))) and ttt.next0_.next0_ is not None and ttt.next0_.next0_.is_char('.')) and not (isinstance(ttt.next0_.next0_.next0_, NumberToken))): 
                        res.numbers.append(str(ttt.next0_.value))
                        res.num_end_token = ttt.next0_.next0_
                        res.end_token = res.num_end_token
                        t = res.end_token
        ...
        if (((isinstance(t, TextToken)) and t.length_char == 1 and t.chars.is_letter) and t == res.begin_token): 
            ...
            if ((t.next0_ is not None and t.next0_.is_hiphen and (isinstance(t.next0_.next0_, TextToken))) and t.next0_.next0_.chars == t.chars): 
                ...
                return
            if (t.next0_ is not None and t.next0_.is_char_of("(.)")): 
                if (t.next0_.is_char('(') and t.is_value("И", None) and not (isinstance(t.next0_.next0_, NumberToken))): 
                    return
                if (((t.next0_.is_char_of("(.") and (isinstance(t.next0_.next0_, NumberToken)) and t.next0_.next0_.next0_ is not None) and t.next0_.next0_.next0_.is_char(')') and not t.next0_.is_whitespace_after) and not t.next0_.next0_.is_whitespace_after): 
                    ...
                    res.num_begin_token = t
                    t = t.next0_.next0_.next0_
                    if ((isinstance(t.next0_, TextToken)) and t.next0_.is_char_of(").")): 
                        t = t.next0_
                        res.num_suffix = t.term
                    res.num_end_token = t
                    res.end_token = res.num_end_token
    ...
    def create_diap(s1 : str, s2 : str) -> typing.List[str]:
        ...
        if (s2.startswith(s1)): 
            ...
            if (((i + 1) < len(s2)) and s2[i] == '.' and str.isdigit(s2[i + 1])): 
                wrapn22172 = RefOutArgWrapper(0)
                inoutres2173 = Utils.tryParseInt(s2[i + 1:], wrapn22172)
                n2 = wrapn22172.value
                if (inoutres2173): 
                    res0 = list()
                    res0.append(s1)
        ...
        if (((i)) > 0): 
            pref = s1[0:0+i + 1]
            wrapn12176 = RefOutArgWrapper(0)
            inoutres2177 = Utils.tryParseInt(s1[i + 1:], wrapn12176)
            n1 = wrapn12176.value
            if (not inoutres2177): 
                return None
            if (not s2.startswith(pref)): 
                return None
            wrapn22174 = RefOutArgWrapper(0)
            inoutres2175 = Utils.tryParseInt(s2[i + 1:], wrapn22174)
            n2 = wrapn22174.value
            if (not inoutres2175): 
                return None
        else: 
            wrapn12180 = RefOutArgWrapper(0)
            inoutres2181 = Utils.tryParseInt(s1, wrapn12180)
            n1 = wrapn12180.value
            if (not inoutres2181): 
                return None
            wrapn22178 = RefOutArgWrapper(0)
            inoutres2179 = Utils.tryParseInt(s2, wrapn22178)
            n2 = wrapn22178.value
            if (not inoutres2179): 
                return None
        if (n2 <= n1): 
