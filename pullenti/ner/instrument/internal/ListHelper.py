...
class ListHelper:
    ...
    class LineToken(MetaToken):
        ...
        def correct_begin_token(self) -> None:
            if (not self.is_list_item): 
                return
            if (self.begin_token.is_hiphen and self.begin_token.next0_ is not None and (self.begin_token.next0_.end_char < self.end_char)): 
                self.begin_token = self.begin_token.next0_
            elif ((self.number > 0 and self.begin_token.next0_ is not None and self.begin_token.next0_.is_char(')')) and self.begin_token.next0_.next0_ is not None and (self.begin_token.next0_.next0_.end_char < self.end_char)): 
                        ...
                        if ((ord(list_bullet)) != 0 and ch.begin_token.is_char(list_bullet)): 
                            ...
                            while tt is not None and (tt.end_char < ch.end_char): 
                                if (tt.previous.is_char('.') and MiscHelper.can_be_start_of_sentence(tt)): 
                                    ch2 = FragToken._new1952(tt, ch.end_token, InstrumentKind.INDENTION, ch.number)
    ...
    def __analize_list_items(chi : typing.List['FragToken'], ind : int) -> int:
        ...
        if (res.has_changes and res.multiline_changes_value is not None): 
            ...
            cit = FragToken._new1935(ci.begin_token, ci.end_token, InstrumentKind.CITATION)
        ...
        for li in lines: 
            ...
            ch = FragToken._new1952(li.begin_token, li.end_token, (InstrumentKind.LISTITEM if li.is_list_item else InstrumentKind.CONTENT), li.number)
            ...
            if (chli is not None): 
                for lt in chli: 
                    ch.children.append(FragToken._new1935(lt.begin_token, lt.end_token, InstrumentKind.LISTITEM))
                if (ch.begin_char < ch.children[0].begin_char): 
                    ch.children.insert(0, FragToken._new1935(ch.begin_token, ch.children[0].begin_token.previous, InstrumentKind.CONTENT))
        return ret
    ...
    def correct_app_list(lines : typing.List['InstrToken1']) -> None:
            ...
            if ((lines[i].typ == InstrToken1.Types.LINE and len(lines[i].numbers) == 0 and lines[i].begin_token.is_value("ПРИЛОЖЕНИЯ", "ДОДАТОК")) and len(lines[i + 1].numbers) > 0 and lines[i].end_token.is_char(':')): 
                ...
                while True:
                    ...
                    if (len(lines[i].numbers) == 0): 
                        ...
                        nn = 0
                        wrapnn2162 = RefOutArgWrapper(0)
                        inoutres2163 = Utils.tryParseInt(lines[i].numbers[0], wrapnn2162)
                        nn = wrapnn2162.value
                        if (inoutres2163): 
                            num = (nn + 1)
        ...
        while True:
            ...
            if (not (i < len(lines))): break
            line = lines[i]
            if (line.typ != InstrToken1.Types.APPENDIX): 
                continue
            apps += 1
            j = 0
            j = (i + 1)
            while j < len(lines): 
                if (lines[j].typ == InstrToken1.Types.APPENDIX): 
                    break
                j += 1
            if (j >= len(lines)): 
                break
            if ((j - i) < 3): 
                empty_apps += 1
            i = (j - 1)
        if (empty_apps > 1 and empty_apps == (apps - 1)): 
            i = 0
            first_pass4119 = True
            while True:
                ...
                if (line.typ != InstrToken1.Types.APPENDIX): 
                    continue
                line.typ = InstrToken1.Types.LINE
                line.numbers.clear()
                line.num_typ = NumberTypes.UNDEFINED
    ...
    def correct_index(lines : typing.List['InstrToken1']) -> None:
        ...
        if (len(index) == len(content) and len(index) > 2): 
            if ((ind_text * 10) < con_text): 
                lines[0] = InstrToken1._new2164(lines[0].begin_token, lines[cind - 1].end_token, True, InstrToken1.Types.INDEX)
