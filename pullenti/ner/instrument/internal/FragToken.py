...
class FragToken(MetaToken):
    ...
    def _get_restored_name(b : 'Token', e0_ : 'Token', index_item : bool=False) -> str:
        ...
        if (not Utils.isNullOrEmpty(str0_)): 
            if (BracketHelper.is_bracket_char(str0_[0], True) and BracketHelper.is_bracket_char(str0_[len(str0_) - 1], True)): 
                str0_ = str0_[1:1+len(str0_) - 2]
    ...
    def _create_referent(self, ad : 'AnalyzerData', bas : 'FragToken') -> 'InstrumentBlockReferent':
        ...
        if (self._m_doc is not None): 
            ...
            tnum = self.__calc_tail_number()
            if (tnum is not None): 
                res.add_slot(InstrumentBlockReferent.ATTR_TAILNUMBER, tnum, False, 0)
            if (self.is_expired): 
                res.is_expired = True
            if (self.is_error): 
                res.is_error = True
    ...
    def fill_by_content_children(self) -> None:
        self.sort_children()
        if (len(self.children) == 0): 
            self.children.append(FragToken._new1935(self.begin_token, self.end_token, InstrumentKind.CONTENT))
            return
        if (self.begin_char < self.children[0].begin_char): 
            self.children.insert(0, FragToken._new1935(self.begin_token, self.children[0].begin_token.previous, InstrumentKind.CONTENT))
        i = 0
        while i < (len(self.children) - 1): 
            if (self.children[i].end_token.next0_ != self.children[i + 1].begin_token and (self.children[i].end_token.next0_.end_char < self.children[i + 1].begin_char)): 
                self.children.insert(i + 1, FragToken._new1935(self.children[i].end_token.next0_, self.children[i + 1].begin_token.previous, InstrumentKind.CONTENT))
            i += 1
        if (self.children[len(self.children) - 1].end_char < self.end_char): 
            self.children.append(FragToken._new1935(self.children[len(self.children) - 1].end_token.next0_, self.end_token, InstrumentKind.CONTENT))
    ...
    def create_document(t : 'Token', max_char : int, root_kind : 'InstrumentKind'=InstrumentKind.UNDEFINED) -> 'FragToken':
        ...
        while (isinstance(t, TextToken)) and t.next0_ is not None:
            tt = MiscHelper.check_image(t)
            if (tt is not None): 
                t = tt.next0_
                continue
            if (t.is_table_control_char or not t.chars.is_letter or t.is_value("КОНСУЛЬТАНТПЛЮС", None)): 
                t = t.next0_
            elif (t.is_value2("ДОКУМЕНТ", "ПРЕДОСТАВЛЕН")): 
                t = t.next0_.next0_
            else: 
                break
        cou1 = 10
        ...
        while True:
            ...
            if (not (tt is not None and cou1 > 0)): break
            if (not tt.is_newline_before): 
                continue
            cou1 -= 1
            if (tt.is_value3("ДОКУМЕНТ", "ПРЕДОСТАВЛЕН", "КОНСУЛЬТАНТПЛЮС")): 
                tt = tt.next0_.next0_
                t = tt
                cons = True
                continue
            if (tt.is_value2("ДАТА", "СОХРАНЕНИЯ")): 
                tt = tt.next0_.next0_
                if (tt is not None and tt.is_char(':')): 
                    tt = tt.next0_
                if (tt is not None and (isinstance(tt.get_referent(), DateReferent))): 
                    tt = tt.next0_
                t = tt
                continue
        if (t is None): 
        ...
        if (res is not None): 
            return res
        res = FragToken._new1935(t, t, InstrumentKind.DOCUMENT)
        ...
        if (head1 is not None): 
            ...
            if (head is None): 
                ...
                ee = True
            elif (len(head1.children) > len(head.children) and head1.end_char > head.end_char): 
                ee = True
            if (ee): 
                head = head1
                res._m_doc = app_doc
        if (head is not None): 
            ...
            if (len(head.children) > 1 and head.children[len(head.children) - 1].kind == InstrumentKind.IGNORED): 
                last = head.children[len(head.children) - 1]
                res.children.append(last)
                head.children.remove(last)
                head.end_token = last.begin_token.previous
        ...
        for i in range(i1 - 1, 0, -1):
            ...
            if (lii.has_table_chars): 
                if ((i < (i1 - 1)) and lii.typ != ILTypes.PERSON and not lii.no_words): 
                    break
                if (is_contract): 
                    break
            if ((lii.typ == ILTypes.PERSON or lii.typ == ILTypes.REGNUMBER or lii.typ == ILTypes.DATE) or lii.typ == ILTypes.GEO): 
                ...
                if (not lii.is_newline_before and not lii.begin_token.is_table_control_char and ((lii.typ == ILTypes.GEO or li[i].typ == ILTypes.PERSON))): 
                    if (i > 0 and ((li[i - 1].typ == ILTypes.UNDEFINED and not li[i - 1].end_token.is_table_control_char))): 
                        if (li[i - 1].begin_token.is_value("ИСП", None) or li[i - 1].begin_token.is_value("ИСПОЛНИТЕЛЬ", None)): 
                            pass
                        else: 
                            break
                    ...
                    if (not lii.is_newline_after): 
                        if (lii.end_token.next0_ is not None and not lii.end_token.next0_.is_table_control_char): 
                            break
                if (tail is None): 
                    tail = FragToken._new1935(li[i].begin_token, li[i1 - 1].end_token, InstrumentKind.TAIL)
                    if ((i1 - 1) > i): 
                        pass
                ...
                if (li[i].is_podpis_storon and tail is not None): 
                    ...
                    tail.children.insert(0, FragToken._new1992(li[i].begin_token, li[i].end_token, InstrumentKind.NAME, ss))
                    cmax = (i - 1)
                    break
                jj = 0
                jj = 0
                ...
                if (jj >= len(ss)): 
                    continue
                if ((len(ss) < 100) and (((i1 - i) < 5))): 
                    continue
                if ((len(ss) < 15) and (((i1 - i) < 10))): 
                    continue
                break
        else: i = 0
        if (cmax < 0): 
            ...
            content = FragToken._new1935(li[0].begin_token, li[cmax].end_token, InstrumentKind.CONTENT)
        ...
        if (tail is not None): 
            ...
            while i1 < len(li): 
                if (li[i1].begin_token == li[i1].end_token and (isinstance(li[i1].begin_token.get_referent(), DecreeReferent)) and li[i1].begin_token.get_referent().kind == DecreeKind.PUBLISHER): 
                    ap = FragToken._new1935(li[i1].begin_token, li[i1].end_token, InstrumentKind.APPROVED)
                    ap.referents = list()
            ...
            if (len(tail.children) > 0 and (tail.children[len(tail.children) - 1].end_char < tail.end_char)): 
                unkw = FragToken._new1935(tail.children[len(tail.children) - 1].end_token.next0_, tail.end_token, InstrumentKind.UNDEFINED)
        ...
        while i < len(li): 
            ...
            if (li[j - 1].typ == ILTypes.PERSON and li[j - 1].is_newline_before and li[j - 1].is_newline_after): 
                tail = FragToken._new1935(li[j - 1].begin_token, li[j - 1].end_token, InstrumentKind.TAIL)
                for jj in range(j - 1, i, -1):
                    if (li[jj].typ != ILTypes.PERSON or not li[jj].is_newline_before or not li[jj].is_newline_after): 
                        break
                    else: 
                        fr = FragToken._new1935(li[jj].begin_token, li[jj].end_token, InstrumentKind.SIGNER)
            ...
            if (title is None): 
                ...
                if (is_app and app.kind == InstrumentKind.APPENDIX): 
                    ...
                    res0 = FragToken._new1998(res.begin_token, res.end_token, res._m_doc, InstrumentKind.DOCUMENT)
                    res._m_doc = (None)
                ...
                if ((app0 is not None and not is_app and app0.kind == InstrumentKind.INTERNALDOCUMENT) and app.kind == InstrumentKind.APPENDIX): 
                    ...
                    res.children.append(app)
                if (not is_app and app.kind == InstrumentKind.INTERNALDOCUMENT): 
                    app0 = app
                ...
                if (title.end_token.next0_ is not None): 
                    if (title.end_token.end_char < app.end_token.begin_char): 
                        acontent = FragToken._new1935(title.end_token.next0_, app.end_token, InstrumentKind.CONTENT)
    ...
    def __create_case_info(t : 'Token') -> 'FragToken':
        ...
        if ((not has_date and t1.next0_ is not None and (isinstance(t1.next0_.get_referent(), DateReferent))) and t1.next0_.is_newline_after): 
            t1 = t1.next0_
        return FragToken._new1935(t, t1, InstrumentKind.CASEINFO)
    ...
    def __create_approved(t : 'Token') -> 'FragToken':
        ...
        if (((isinstance(t, ReferentToken)) and t.begin_token.is_char('(') and t.end_token.is_char(')')) and t.begin_token.next0_.is_value("ПРОТОКОЛ", None)): 
            res = FragToken._new1935(t, t, InstrumentKind.APPROVED)
            res.referents = list()
            res.referents.append(t.get_referent())
            return res
        tt = InstrToken._check_approved(t)
        if (tt is not None): 
            res = FragToken._new1935(t, tt, InstrumentKind.APPROVED)
        ...
        if (dts is not None and len(dts) > 0): 
            ...
            while i < len(dts): 
                ...
                if (dt.typ == DecreeToken.ItemType.ORG): 
                    res.children.append(FragToken._new1992(dt.begin_token, dt.end_token, InstrumentKind.ORGANIZATION, dt))
                elif (dt.typ == DecreeToken.ItemType.OWNER): 
                    res.children.append(FragToken._new1992(dt.begin_token, dt.end_token, InstrumentKind.INITIATOR, dt))
                elif (dt.typ == DecreeToken.ItemType.DATE): 
                    res.children.append(FragToken._new1992(dt.begin_token, dt.end_token, InstrumentKind.DATE, dt))
                elif (dt.typ == DecreeToken.ItemType.NUMBER and i > 0 and dts[i - 1].typ == DecreeToken.ItemType.DATE): 
                    res.children.append(FragToken._new1992(dt.begin_token, dt.end_token, InstrumentKind.NUMBER, dt))
                    ...
                    res.children.append(FragToken._new1992(dt.begin_token, dt.end_token, InstrumentKind.PLACE, dt))
                else: 
                    break
    ...
    def _create_misc(t : 'Token') -> 'FragToken':
        ...
        if (t.is_value("ФОРМА", None) and t.next0_.is_value("ДОКУМЕНТА", None)): 
            ...
            if (num is not None and num.typ == DecreeToken.ItemType.NUMBER): 
                return FragToken._new1935(t, num.end_token, InstrumentKind.UNDEFINED)
            if ((isinstance(t.next0_.next0_, NumberToken)) and t.next0_.next0_.is_newline_after): 
                return FragToken._new1935(t, t.next0_.next0_, InstrumentKind.UNDEFINED)
        npt = NounPhraseHelper.try_parse(t, NounPhraseParseAttr.NO, 0, None)
        if (npt is not None): 
            if (npt.end_token.is_value("ОБРАЗЕЦ", None) and t.chars.is_all_lower): 
                return FragToken(t, npt.end_token)
        if (t.is_char('(')): 
            npt = NounPhraseHelper.try_parse(t.next0_, NounPhraseParseAttr.NO, 0, None)
            if (npt is not None and npt.end_token.next0_ is not None and npt.end_token.next0_.is_char(')')): 
                return FragToken._new1935(t, npt.end_token.next0_, InstrumentKind.UNDEFINED)
        if ((isinstance(t, NumberToken)) and t.next0_ is not None and t.next0_.is_value("СЕССИЯ", None)): 
            return FragToken._new1935(t, t.next0_, InstrumentKind.UNDEFINED)
        if (t.is_value("С", None) and t.next0_.is_value("ИЗМЕНЕНИЕ", None) and t.next0_.next0_ is not None): 
            ...
            if (isinstance(tt.get_referent(), DateReferent)): 
                if (tt.next0_ is not None and tt.next0_.is_char('.')): 
                    tt = tt.next0_
                return FragToken._new1935(t, tt, InstrumentKind.UNDEFINED)
        t0 = t
        ...
        if (t.is_value("ЗАКАЗ", None)): 
            ...
            if (itt is not None): 
                return FragToken._new1935(t, itt.end_token, InstrumentKind.UNDEFINED)
        nums = 0
        spec = 0
        chars_ = 0
        words = 0
        ...
        while True:
            ...
            if ((nums + spec + chars_) > (words * 3)): 
                ...
                break
            return FragToken._new1935(t0, t1, InstrumentKind.UNDEFINED)
        if (isinstance(t.get_referent(), StreetReferent)): 
            ...
            if (lin is not None and (lin.length_char < 70)): 
                return FragToken._new1935(t0, lin.end_token, InstrumentKind.CONTACT)
        if (t0.is_char('(')): 
            br = BracketHelper.try_parse(t0, BracketParseAttr.NO, 100)
            if (br is not None and br.is_newline_after): 
                return FragToken._new1935(t0, br.end_token, InstrumentKind.UNDEFINED)
        if (t0.previous is not None and t0.previous.is_table_control_char): 
            rows = TableHelper.try_parse_rows(t0.previous, 0, False)
            if (rows is not None and (len(rows) < 30) and len(rows) > 3): 
                tt = rows[len(rows) - 1].end_token.next0_
                if (tt is not None and tt.is_newline_before): 
                    dt = DecreeToken.try_attach(tt, None, False)
                    if (dt is not None and dt.typ == DecreeToken.ItemType.TYP): 
                        return FragToken._new1935(t0.previous, tt.previous, InstrumentKind.UNDEFINED)
        return None
    ...
    def _create_editions(t : 'Token') -> 'FragToken':
        ...
        if ((t.is_newline_before and t.is_value("С", None) and t.next0_ is not None) and t.next0_.is_value("ИЗМЕНЕНИЕ", None)): 
            eee = FragToken._new1935(t, t, InstrumentKind.EDITIONS)
            t = t.next0_.next0_
            while t is not None: 
        ...
        if (not t.is_char('(') and not t.is_value("ПРИЛОЖЕНИЕ", "ДОДАТОК")): 
            if (t.is_value("В", "У") and t.next0_ is not None and ((t.next0_.is_value("РЕДАКЦИЯ", "РЕДАКЦІЯ") or t.next0_.is_value("РЕД", None)))): 
                ...
                if (dtt0 is not None): 
                    return FragToken._new1935(t, dtt0.end_token, InstrumentKind.EDITIONS)
        ...
        if (t.is_value("РЕДАКЦИЯ", "РЕДАКЦІЯ")): 
            ok = True
        elif (t.is_value("ОТФОРМАТИРОВАН", None) or t.is_value("ДОПОЛНЕН", None) or t.is_value("ДОПОЛНЕНИЕ", None)): 
            ok = True
        ...
        if (ok and not is_doubt): 
            eds = FragToken._new1935(t0, t, InstrumentKind.EDITIONS)
            eds.referents = list()
            for d in decrs: 
    ...
    def __create_owner(t : 'Token') -> 'FragToken':
        ...
        if (t1 is None): 
            return None
        fr = FragToken._new1954(t, t1, InstrumentKind.ORGANIZATION, True)
        return fr
            ...
            if (isinstance(t.get_referent(), DecreeChangeReferent)): 
                ...
                while True:
                    ...
                    if (t0 is not None and ((t0.end_char + 50) < t1.end_char)): 
                        return MetaToken._new657(t0, t1, dcr)
                    return None
            if (t.end_char > self.end_char): 
                break
            t = t.next0_
    ...
    def __calc_tail_number(self) -> str:
        if (self.kind == InstrumentKind.NUMBER or self.kind == InstrumentKind.NAME): 
            return None
        t = self.end_token
        if (t.is_comma): 
            t = t.previous
        if (t is None or not t.is_char_of(")]")): 
            return None
        has_num = False
        tt = t.previous
        first_pass4051 = True
        while True:
            ...
            if (not (tt is not None and tt.begin_char >= self.begin_char)): break
            if (tt.is_char_of("([")): 
                return (MiscHelper.get_text_value(tt.next0_, t.previous, GetTextAttr.NO) if has_num else None)
    ...
    def _analize_tables(self) -> bool:
        ...
        while True:
            ...
            if (t0.end_char < rows[0].begin_char): 
                self.children.append(FragToken._new1935(t0, rows[0].begin_token.previous, InstrumentKind.CONTENT))
            tab = FragToken._new1935(rows[0].begin_token, rows[len(rows) - 1].end_token, InstrumentKind.TABLE)
            self.children.append(tab)
            i = 0
            while i < len(rows): 
                rr = FragToken._new1952(rows[i].begin_token, rows[i].end_token, InstrumentKind.TABLEROW, i + 1)
                ...
                for ce in rows[i].cells: 
                    no += 1
                    cell = FragToken._new1952(ce.begin_token, ce.end_token, InstrumentKind.TABLECELL, no)
                    if (ce.col_span > 1): 
        ...
        if ((t0 is not None and (t0.end_char < self.end_char) and tabs) and t0 != self.end_token): 
            self.children.append(FragToken._new1935(t0, self.end_token, InstrumentKind.CONTENT))
        return tabs
    ...
    def __create_doc_title(t0 : 'Token', doc : 'InstrumentReferent') -> 'FragToken':
        if (t0 is None): 
            return None
        if (t0.is_value3("ДАННЫЕ", "ЭЛЕКТРОННОЙ", "ПОДПИСИ")): 
            cou = 0
            tt = t0
            first_pass4054 = True
            while True:
                ...
                if (not (tt is not None)): break
                t0 = tt
                line = InstrToken1.parse(tt, True, None, 0, None, False, 0, False, False)
                if (line is None): 
                    continue
                if (isinstance(tt.get_referent(), DecreeReferent)): 
                    t0 = tt.kit.debed_token(tt)
                    break
                dt = DecreeToken.try_attach(tt, None, False)
                if (dt is not None and dt.typ == DecreeToken.ItemType.TYP): 
                    break
                if ((((tt.is_value("ДАННЫЕ", None) or tt.is_value("ДОКУМЕНТ", None) or tt.is_value("ВЛАДЕЛЕЦ", None)) or tt.is_value("ОРГАНИЗАЦИЯ", None) or tt.is_value("ПОДПИСАНО", None)) or tt.is_value2("СЕРИЙНЫЙ", "НОМЕР") or tt.is_value2("СРОК", "ДЕЙСТВИЯ")) or tt.is_char('(')): 
                    cou = 0
                else: 
                    cou += 1
                    if (cou > 3): 
        ...
        if ((title is not None and len(title.children) >= 1 and title.children[0].kind == InstrumentKind.NAME) and doc.typ is None): 
            t = title.end_token.next0_
            while t is not None: 
                if (t.is_value("ПРИКАЗЫВАТЬ", None) and t.next0_ is not None and t.next0_.is_char(':')): 
                    doc.typ = "ПРИКАЗ"
                    break
                t = t.next0_
        return title
    ...
    def __create_doc_title_(t0 : 'Token', doc : 'InstrumentReferent') -> 'FragToken':
        ...
        if (t0 is None): 
            return None
        if (FragToken.__is_start_of_body(t0, False)): 
            return None
        title = FragToken._new1935(t0, t0, InstrumentKind.HEAD)
        dt0 = None
        t = None
        t1 = None
        name_ = None
        ...
        if (t is not None and t.is_value("ДЕЛО", "СПРАВА")): 
            ...
            if (dt is not None and dt.typ == DecreeToken.ItemType.NUMBER): 
                dt.begin_token = t
                title.children.append(FragToken._new1992(t, t, InstrumentKind.KEYWORD, "ДЕЛО"))
        ...
        while True:
            ...
            if (t.is_newline_before or ((t.previous is not None and t.previous.is_table_control_char))): 
                ...
                if (iii is not None and iii.typ == InstrToken1.Types.COMMENT): 
                    cmt = FragToken._new1935(iii.begin_token, iii.end_token, InstrumentKind.COMMENT)
                ...
                if (iii is not None and iii.end_token.is_char('?')): 
                    cmt = FragToken._new1935(iii.begin_token, iii.end_token, InstrumentKind.NAME)
                ...
                if ((((t.is_value("ЗАЯВИТЕЛЬ", "ЗАЯВНИК") or t.is_value("ИСТЕЦ", "ПОЗИВАЧ") or t.is_value("ОТВЕТЧИК", "ВІДПОВІДАЧ")) or t.is_value("ДОЛЖНИК", "БОРЖНИК") or t.is_value("КОПИЯ", "КОПІЯ"))) and t.next0_ is not None and ((t.next0_.is_char(':') or t.next0_.is_table_control_char))): 
                    ...
                    if (ptt is not None): 
                        ...
                        if (t1.next0_ is not None and t1.next0_.is_char('(')): 
                            ...
                            if (br is not None): 
                                t1 = br.end_token
                        ft = FragToken._new1935(t, t1, InstrumentKind.INITIATOR)
                ...
                if (t.is_value("ЦЕНА", "ЦІНА") and t.next0_ is not None and t.next0_.is_value("ИСК", "ПОЗОВ")): 
                    ...
                    if (tt is not None and has_money): 
                        ...
                        if (tt.next0_ is not None and tt.next0_.is_char('(')): 
                            ...
                            if (br is not None): 
                                tt = br.end_token
                        title.children.append(FragToken._new1935(t, tt, InstrumentKind.CASEINFO))
                        t1 = tt
                        title.end_token = t1
                        t = title.end_token
                        continue
                if (t.is_value("В", "У")): 
                    ...
                    if (tt is not None and (isinstance(tt.get_referent(), OrganizationReferent))): 
                        ...
                        if (t1.next0_ is not None and t1.next0_.is_char('(')): 
                            ...
                            if (br is not None): 
                                t1 = br.end_token
                        ooo = FragToken._new1935(t, t1, InstrumentKind.ORGANIZATION)
                        ooo.referents = list()
                ...
                if (t.length_char == 1 and t.chars.is_letter and t.is_whitespace_after): 
                    ...
                    while ii < len(InstrToken._m_directives_norm): 
                        ...
                        if (ee is not None and ee.is_newline_after): 
                            ooo = FragToken._new1992(t, ee, InstrumentKind.KEYWORD, InstrToken._m_directives_norm[ii])
                            title.children.append(ooo)
            ...
            if (dr0 is not None): 
                if (doc.typ is None or doc.typ == dr0.typ): 
                    ...
                    if (li is not None and len(li) > 0 and li[len(li) - 1].is_newline_after): 
                        ...
                        if (doc.typ == "КОДЕКС"): 
                            ...
                            if (pt is not None and len(pt.values) > 0): 
                                ...
                                title.children.append(FragToken._new1992(pt.begin_token, pt.end_token, InstrumentKind.DOCPART, pt.values[0].value))
                                t = pt.end_token
                                continue
                    ...
                    t1 = t
                    title.children.append(FragToken._new1992(t, t, InstrumentKind.NAME, nam))
                    if (pt is not None and len(pt.values) > 0): 
                        title.children.append(FragToken._new1992(pt.begin_token, pt.end_token, InstrumentKind.DOCPART, pt.values[0].value))
                        t1 = pt.end_token
                    t = t1
            ...
            if (doc.typ is None): 
                ...
                if (ttt1 is not None and ttt1.is_newline_after): 
                    ...
                    if (doc.typ is None): 
                        doc.typ = typ
                    title.children.append(FragToken._new1992(t, ttt1, InstrumentKind.TYP, typ))
                    dt0 = DecreeToken._new1092(t, ttt1, DecreeToken.ItemType.TYP, typ)
                    can_be_orgs = False
                    t = ttt1
                    t1 = t
                    if ((t.next0_ is not None and t.next0_.is_newline_after and typ == "УСТАВ") and (((isinstance(t.next0_.get_referent(), GeoReferent)) or (isinstance(t.next0_.get_referent(), OrganizationReferent))))): 
                        t = ttt1.next0_
                        t1 = t
                        nam = MiscHelper.get_text_value_of_meta_token(Utils.asObjectOrNull(t, MetaToken), GetTextAttr.RESTOREREGISTER)
                        doc.add_slot(DecreeReferent.ATTR_NAME, "Устав " + nam, False, 0)
                        title.children.append(FragToken._new1992(t, t, InstrumentKind.NAME, nam))
                    continue
            ...
            if (dt is None and dt0 is not None and ((dt0.typ == DecreeToken.ItemType.OWNER or dt0.typ == DecreeToken.ItemType.ORG))): 
                if ((isinstance(t, NumberToken)) and t.is_newline_after and t.is_newline_before): 
                    dt = DecreeToken._new1092(t, t, DecreeToken.ItemType.NUMBER, str(t.value))
            ...
            if (dt is None): 
                ...
                if (t.is_value("ДАТА", None)): 
                    ...
                    while True:
                        ...
                        if (da is not None): 
                            frdt = FragToken._new1935(t, tt, InstrumentKind.DATE)
                ...
                if (((isinstance(r, AddressReferent)) or (isinstance(r, UriReferent)) or (isinstance(r, PhoneReferent))) or (isinstance(r, PersonIdentityReferent)) or (isinstance(r, BankDataReferent))): 
                    cnt = FragToken._new1935(t, t, InstrumentKind.CONTACT)
                    cnt.referents = list()
            ...
            if (t == t0): 
                ...
                if (nam is not None): 
                    ...
                    if (not Utils.isNullOrEmpty(name_)): 
                        ...
                        title.children.append(FragToken._new1992(t0, t1, InstrumentKind.NAME, name_.strip()))
            ...
            if (t is not None and t.is_value("О", None)): 
                ...
                if (nam is not None): 
                    ...
                    if (not Utils.isNullOrEmpty(name_)): 
                        t1 = nam.end_token
                        doc.add_slot(InstrumentBlockReferent.ATTR_NAME, name_.strip(), True, 0)
                        title.children.append(FragToken._new1992(t, t1, InstrumentKind.NAME, name_.strip()))
            if (attrs > 0): 
                title.end_token = t1
                return title
            return None
        ...
        if (end_empty_lines is not None and doc.find_slot(InstrumentReferent.ATTR_SOURCE, None, True) is None): 
            ...
            title.children.insert(0, FragToken._new1992(t0, end_empty_lines, InstrumentKind.ORGANIZATION, val))
        is_case = False
        ...
        if ((((name_ is not None or t.is_newline_before or ((t.previous is not None and t.previous.is_table_control_char))) or ((not t.is_newline_before and len(title.children) > 0 and title.children[len(title.children) - 1].kind == InstrumentKind.TYP)))) and not is_case): 
            ...
            if (val is not None and tt0.begin_char <= t1.begin_char): 
                if (nt0 is not None): 
                    tt0 = nt0
                val = val.strip()
                ...
                title.children.append(FragToken._new1992(tt0, t1, InstrumentKind.NAME, val.strip()))
                if ("КОДЕКС" in val): 
                    npt = NounPhraseHelper.try_parse(tt0, NounPhraseParseAttr.NO, 0, None)
        ...
        while True:
            ...
            if (t1.is_newline_before and (isinstance(t1.get_referent(), DecreeReferent)) and t1.is_newline_after): 
                dr = Utils.asObjectOrNull(t1.get_referent(), DecreeReferent)
                title.children.append(FragToken._new1935(t1, t1, InstrumentKind.IGNORED))
                continue
            if (t1.is_newline_before and t1.is_value("ЧАСТЬ", "ЧАСТИНА")): 
                ...
                if (pt is not None and pt.is_newline_after): 
                    ...
                    if (pt2 is not None and (((pt2.typ == PartToken.ItemType.SECTION or pt2.typ == PartToken.ItemType.SUBSECTION or pt2.typ == PartToken.ItemType.CHAPTER) or pt2.typ == PartToken.ItemType.CLAUSE))): 
                        ...
                        doc.add_slot(InstrumentReferent.ATTR_PART, pt.values[0].value, False, 0)
                        title.children.append(FragToken._new1992(t1, pt.end_token, InstrumentKind.DOCPART, pt.values[0].value))
            ...
            if (t1.is_newline_before): 
                ...
                if (iii is not None and iii.typ == InstrToken1.Types.COMMENT): 
                    title.children.append(FragToken._new1935(t1, iii.end_token, InstrumentKind.COMMENT))
                    t1 = iii.end_token
                    continue
            ...
            if ((isinstance(t1.get_referent(), DecreeReferent)) and t1.get_referent().kind == DecreeKind.PUBLISHER and t1.is_newline_after): 
                pub = FragToken._new1935(t1, t1, InstrumentKind.APPROVED)
                pub.referents = list()
            ...
            if (tt.is_char_of("([") and tt.is_newline_before): 
                ...
                if (br is not None): 
                    ...
                    t1 = title.end_token
                    title.children.append(FragToken._new1935(br.begin_token, br.end_token, (InstrumentKind.NAME if tt.is_char('[') else InstrumentKind.COMMENT)))
                    continue
    ...
    def __create_appendix_title(t0 : 'Token', app : 'FragToken', doc : 'InstrumentReferent', is_app : bool, start : bool) -> 'FragToken':
        ...
        if (rr is not None): 
            if (rr.type_name == "PERSON"): 
                return None
        title = FragToken._new1935(t0, t, InstrumentKind.HEAD)
        ...
        while True:
            ...
            if (fr is None): 
                break
            if (fr.typ == InstrToken1.Types.COMMENT): 
                t = fr.end_token
                title.end_token = t
                title.children.append(FragToken._new1935(fr.begin_token, fr.end_token, InstrumentKind.COMMENT))
                continue
            ...
            if (t.is_value("ОСОБЫЙ", "ОСОБЛИВИЙ") and t.next0_ is not None): 
                t2 = t.next0_
            if (isinstance(t, TextToken)): 
                title.children.append(FragToken._new1954(t, t2, InstrumentKind.KEYWORD, True))
            t = fr.end_token
            title.end_token = t
            if (fr.typ == InstrToken1.Types.APPENDIX and fr.num_begin_token is None): 
                ...
                if (fr1 is not None and fr1.typ == InstrToken1.Types.APPROVED): 
                    t = fr1.begin_token
                    title.children.append(FragToken._new1992(t, t, InstrumentKind.KEYWORD, t.get_source_text().upper()))
                    t = fr1.end_token
                    title.end_token = t
                    fr = fr1
            appr0 = FragToken.__create_approved(t)
            ...
            if (fr.num_begin_token is not None and fr.num_end_token is not None): 
                num = FragToken._new1992(fr.num_begin_token, fr.num_end_token, InstrumentKind.NUMBER, MiscHelper.get_text_value(fr.num_begin_token, fr.num_end_token, GetTextAttr.KEEPREGISTER))
                ...
                if (t.get_referent().kind == DecreeKind.PUBLISHER): 
                    ff = FragToken._new1935(t, t, InstrumentKind.APPROVED)
                    ff.referents = list()
                    ...
                    appr = FragToken._new1935(kw.begin_token, t, InstrumentKind.APPROVED)
                    del title.children[len(title.children) - 1]
                    appr.children.append(kw)
                    appr.children.append(FragToken._new1935(t, t, InstrumentKind.DOCREFERENCE))
                    title.children.append(appr)
                else: 
                    title.children.append(FragToken._new1935(t, t, InstrumentKind.DOCREFERENCE))
            elif (fr.typ == InstrToken1.Types.APPROVED and fr.length_char > 15 and fr.begin_token != fr.end_token): 
                title.children.append(FragToken._new1935(fr.begin_token.next0_, t, InstrumentKind.DOCREFERENCE))
            else: 
                ...
                if (dts is not None and len(dts) > 0 and dts[0].typ == DecreeToken.ItemType.TYP): 
                    dref = FragToken._new1935(dts[0].begin_token, dts[0].end_token, InstrumentKind.DOCREFERENCE)
                    i = 1
                    while i < len(dts): 
        ...
        if (t.is_value("К", "ДО")): 
            ...
            while ii < len(toks): 
                tok = toks[ii]
                if (tok.typ == ILTypes.TYP and ((tok.value == doc.typ or ii == 0)) and not tok.begin_token.previous.is_value("К", None)): 
                    max_ind = ii
                    ...
                    if (tok.begin_token.is_value("ОТ", None)): 
                        max_ind = ii
            ...
            if (len(toks) > 0 and DecreeToken.is_keyword(toks[len(toks) - 1].end_token.next0_, False) is not None): 
                max_ind = (len(toks) - 1)
            elif (len(toks) > 0 and (max_ind < 0) and toks[0].begin_token.previous.is_value("К", None)): 
                max_ind = (len(toks) - 1)
            te = None
            if (max_ind >= 0): 
                ...
                te = t.next0_
            if (te is not None): 
                dr = FragToken._new1935(t, te, InstrumentKind.DOCREFERENCE)
        ...
        for kk in range(10):
            ...
            if (ta is not None): 
                ...
                if (t is None): 
                    return title
                continue
            if (t.is_value("РИСУНОК", None)): 
                fr = InstrToken1.parse(t, True, None, 0, None, False, 0, True, False)
                if (fr is None): 
                    break
                if (fr.length_char < 20): 
                    t = fr.end_token
                    title.end_token = t
                    t = t.next0_
                    continue
        ...
        while True:
            ...
            if (t.is_newline_before or t.previous.is_table_control_char): 
                ...
                if (ltt.typ == InstrToken1.Types.APPROVED): 
                    title.children.append(FragToken._new1935(ltt.begin_token, ltt.begin_token, InstrumentKind.APPROVED))
                    if (ltt.begin_token != ltt.end_token): 
                        title.children.append(FragToken._new1935(ltt.begin_token.next0_, ltt.end_token, InstrumentKind.DOCREFERENCE))
                    t = ltt.end_token
        ...
        while title.end_token.next0_ is not None:
            ...
            if (appr is not None): 
                ...
                continue
            misc = FragToken._create_misc(title.end_token.next0_)
            if (misc is not None): 
                title.children.append(misc)
                title.end_token = misc.end_token
                continue
            break
        if (is_app): 
    ...
    def __is_start_of_body(t : 'Token', is_app_title : bool=False) -> bool:
        if (t is None or not t.is_newline_before): 
            return False
        if (t.is_value("ТАБЛИЦА", None) or t.is_value("РИСУНОК", None)): 
            num = ComplexNumToken.try_parse(t.next0_, None, True, False)
            if (num is not None): 
                return True
        if (not is_app_title): 
    ...
    def __add_title_attr(doc : 'InstrumentReferent', title : 'FragToken', dt : 'DecreeToken') -> bool:
        if (dt.typ == DecreeToken.ItemType.TYP): 
            ...
            if (title is not None): 
                title.children.append(FragToken._new1992(dt.begin_token, dt.end_token, InstrumentKind.TYP, Utils.ifNotNull(dt.full_value, dt.value)))
            ...
            if (dt.is_delo): 
                if (doc is not None): 
                    ...
                    if (doc.reg_number == dt.value): 
                        doc.reg_number = None
                if (title is not None): 
                    title.children.append(FragToken._new1992(dt.begin_token, dt.end_token, InstrumentKind.CASENUMBER, dt.value))
            else: 
                ...
                if (title is not None): 
                    title.children.append(FragToken._new1992(dt.begin_token, dt.end_token, InstrumentKind.NUMBER, dt.value))
            ...
            if (title is not None): 
                title.children.append(FragToken._new1992(dt.begin_token, dt.end_token, InstrumentKind.NAME, dt.value))
        elif (dt.typ == DecreeToken.ItemType.DATE): 
            if (doc is None or doc._add_date(dt)): 
                if (title is not None): 
                    title.children.append(FragToken._new1992(dt.begin_token, dt.end_token, InstrumentKind.DATE, dt))
        elif (dt.typ == DecreeToken.ItemType.TERR): 
            if (title is not None): 
                title.children.append(FragToken._new1992(dt.begin_token, dt.end_token, InstrumentKind.PLACE, dt))
            ...
            if (title is not None): 
                title.children.append(FragToken._new1992(dt.begin_token, dt.end_token, (InstrumentKind.ORGANIZATION if dt.typ == DecreeToken.ItemType.ORG else InstrumentKind.INITIATOR), dt))
            if (doc is not None): 
        ...
        while t is not None and (cou < 30): 
            ...
            if (t.is_value("ОТВЕТ", None)): 
                if (t.is_newline_after): 
                    ...
                    ok = True
            if (ok): 
                res = FragToken._new1935(t0, li.end_token, InstrumentKind.HEAD)
                if (li.begin_token != t0): 
                    hh = FragToken._new1935(t0, li.begin_token.previous, InstrumentKind.APPROVED)
                    res.children.append(hh)
                res.children.append(FragToken._new1957(li.begin_token, li.end_token, InstrumentKind.KEYWORD, True))
                return res
            t = li.end_token
            t = t.next0_; cou += 1
        return None
    ...
    def __create_contract_title(t0 : 'Token', doc : 'InstrumentReferent') -> 'FragToken':
        ...
        if (t1 is None): 
            return None
        p1 = InstrumentParticipantReferent._new2086(par1.typ)
        ...
        if (par1.kind == ParticipantToken.Kinds.NAMEDASPARTS): 
            ...
            while ii < len(par1.parts): 
                pp = InstrumentParticipantReferent._new2086("СТОРОНА {0}".format(ii + 1))
        ...
        while True:
            ...
            if (isinstance(t.get_referent(), DecreeReferent)): 
                if (t.is_newline_before or ((t.previous is not None and t.previous.is_table_control_char))): 
                    if (t.previous is not None and ((t.previous.is_value("ПРИНЯТ", None) or t.previous.is_value("ДЕЙСТВИЕ", None)))): 
                        pass
                    else: 
                        t = t.kit.debed_token(t)
            ...
            if (new_line_bef and t != t0): 
                ...
                if (isinstance(t, NumberToken)): 
                    cnum = ComplexNumToken.try_parse(t, None, False, False)
                    if (cnum is not None and len(cnum.nums) == 1 and cnum.suffix is not None): 
            ...
            if ((dttyp is not None and nam_beg is None and t.chars.is_cyrillic_letter) and (isinstance(t, TextToken))): 
                if (t.is_value("МЕЖДУ", "МІЖ")): 
                    ...
                    if (pp is not None and pp.end_token.next0_ is not None and pp.end_token.next0_.is_and): 
                        ...
                        if (pp2 is not None): 
                            fr = FragToken._new1935(t, pp2.end_token, InstrumentKind.PLACE)
        ...
        if (nam_end is not None and nam_beg is not None): 
            ...
            if (val is not None and len(val) > 3): 
                nam = FragToken._new1992(nam_beg, nam_end, InstrumentKind.NAME, val)
                ...
                if (nam_beg2 is not None and nam_end2 is not None): 
                    ...
                    if (val2 is not None and len(val2) > 3): 
                        nam = FragToken._new1992(nam_beg2, nam_end2, InstrumentKind.NAME, val2)
                        title.children.append(nam)
                        title.sort_children()
        ...
        if (len(title.children) > 0 and title.children[0].begin_char > title.begin_char): 
            title.children.insert(0, FragToken._new1935(title.begin_token, title.children[0].begin_token.previous, InstrumentKind.UNDEFINED))
        ...
        if (doc.typ is None and len(contr_typs) > 0): 
            doc.typ = contr_typs[0]
        if (len(contr_typs) == 0 and (title.length_char < 20)): 
            return None
    ...
    def __create_project_title(t0 : 'Token', doc : 'InstrumentReferent') -> 'FragToken':
        ...
        while True:
            ...
            if ((isinstance(t, TextToken)) and ((t.term == "ПРОЕКТ" or t.term == "ЗАКОНОПРОЕКТ"))): 
                if ((t.is_value("ПРОЕКТ", None) and t == t0 and (isinstance(t.next0_, ReferentToken))) and (isinstance(t.next0_.get_referent(), OrganizationReferent))): 
                    return None
                is_project = True
                title.children.append(FragToken._new1954(t, t, InstrumentKind.KEYWORD, True))
                doc.add_slot(InstrumentReferent.ATTR_TYPE, "ПРОЕКТ", False, 0)
                continue
            tt = FragToken.__attach_project_enter(t)
            if (tt is not None): 
                is_entered = True
                title.children.append(FragToken._new1935(t, tt, InstrumentKind.APPROVED))
                t = tt
                continue
            tt = FragToken.__attach_project_misc(t)
            if (tt is not None): 
                title.children.append(FragToken._new1935(t, tt, (InstrumentKind.EDITIONS if tt.is_value("ЧТЕНИЕ", "ЧИТАННЯ") else InstrumentKind.UNDEFINED)))
                t = tt
                continue
        ...
        if (t11 is None): 
            return None
        nam = FragToken._new1954(t00, t11, InstrumentKind.NAME, True)
    ...
    def __create_justice_participants(title : 'FragToken', doc : 'InstrumentReferent') -> None:
        ...
        while t is not None:
            ...
            if (tmp.tell() > 10 and t1 is not None): 
                art = InstrumentArtefactReferent._new2101("предмет")
    ...
    def __create_just_participant(t : 'Token', typ : str) -> 'ReferentToken':
        ...
        if (not ok): 
            return None
        pat = InstrumentParticipantReferent._new2086(typ)
        for r in refs: 
            ...
            while True:
                ...
                if (t.is_value("ВЗЫСКАТЬ", "СТЯГНУТИ")): 
                    ...
                    if (sum0_ is not None): 
                        art = InstrumentArtefactReferent._new2101("РЕЗОЛЮЦИЯ")
                        if (gosposh): 
            ...
            if (self.children[i].kind == InstrumentKind.DIRECTIVE and ((i + 1) < len(self.children))): 
                ...
                if ((((s == "РЕШЕНИЕ" or s == "ОПРЕДЕЛЕНИЕ" or s == "ПОСТАНОВЛЕНИЕ") or s == "ПРИГОВОР" or s == "РІШЕННЯ") or s == "ВИЗНАЧЕННЯ" or s == "ПОСТАНОВА") or s == "ВИРОК"): 
                    ...
                    if (ii == (i + 1)): 
                        ...
                        res.append(FragToken._new1935(self.children[i + 1].begin_token, self.children[ii].end_token, InstrumentKind.CONTENT))
                else: 
                    dir0_ = True
        if (res is not None): 
            return res
    ...
    def __create_action_question(t : 'Token', max_char : int) -> 'FragToken':
        ...
        if (res is None): 
            return None
        ques = FragToken._new1935(t, t2.previous, InstrumentKind.QUESTION)
        res.children.insert(0, ques)
        ques.children.append(FragToken._new1935(li[len(li) - 1].begin_token, li[len(li) - 1].end_token, InstrumentKind.KEYWORD))
        content = FragToken._new1935(li[len(li) - 1].end_token.next0_, t2.previous, InstrumentKind.CONTENT)
        ques.children.append(content)
        content._analize_content(res, max_char > 0, InstrumentKind.UNDEFINED)
        if (len(li) > 1): 
            fr = FragToken._new1957(t, li[len(li) - 2].end_token, InstrumentKind.NAME, True)
    ...
    def __create_gost_title(t0 : 'Token', doc : 'InstrumentReferent') -> 'FragToken':
        ...
        while True:
            ...
            if (not (t is not None and (cou < 300))): break
            dr = Utils.asObjectOrNull(t.get_referent(), DecreeReferent)
            if (dr is not None and t.is_newline_before): 
                if (dr.kind == DecreeKind.STANDARD): 
                    ...
                    ok = True
            if (t.is_newline_before or ((t.previous is not None and t.previous.is_table_control_char))): 
                if (((t.is_value2("ГОСУДАРСТВЕННЫЙ", "СТАНДАРТ") or t.is_value2("НАЦИОНАЛЬНЫЙ", "СТАНДАРТ"))) and t.next0_.next0_ is not None and (isinstance(t.next0_.next0_.get_referent(), GeoReferent))): 
                    ok = True
                    break
            if (t.is_table_control_char): 
                continue
        ...
        while True:
            ...
            if (dt.typ == DecreeToken.ItemType.TYP): 
                ...
                FragToken.__add_title_attr(doc, title, dt)
                t = dt.end_token
                if (dt.src_ref is not None): 
                    has_name = True
                    name_ = MiscHelper.get_text_value_of_meta_token(dt, GetTextAttr.RESTOREREGISTER)
                    doc.add_slot(DecreeReferent.ATTR_NAME, name_, False, 0)
                    title.children[len(title.children) - 1].kind = InstrumentKind.NAME
                    title.children[len(title.children) - 1].value = (name_)
                if (not has_num): 
                    ...
                    if (num is not None and num.typ == DecreeToken.ItemType.NUMBER): 
                        ...
                        has_num = True
                        continue
                if ((not has_name and t.is_newline_after and (isinstance(t.next0_, TextToken))) and t.next0_.length_char > 3 and t.next0_.chars.is_cyrillic_letter): 
                    nam = DecreeToken.try_attach_name_to_end_of_line(t.next0_, True)
                    if (nam is not None and nam.length_char > 20): 
                        ...
                        while tt is not None: 
                            ...
                            if (tt.chars.is_latin_letter): 
                                break
                            nam2 = DecreeToken.try_attach_name_to_end_of_line(tt, True)
                            if (nam2 is None or (nam2.length_char < 20)): 
                                break
                            nam.value = "{0}. {1}".format(nam.value, nam2.value)
                            nam.end_token = nam2.end_token
                            tt = nam.end_token
                            tt = tt.next0_
                        FragToken.__add_title_attr(doc, title, nam)
                        t = nam.end_token
                        has_name = True
                        continue
                continue
        ...
        t = title.end_token.next0_
        while t is not None: 
            if (t.begin_char > 10000): 
                break
            if (t.is_newline_before and t.is_value("ПРЕДИСЛОВИЕ", None) and t.is_newline_after): 
                empty = FragToken._new1935(t, t, InstrumentKind.IGNORED)
                title.children.append(empty)
            li = InstrToken1.parse(t, True, None, 0, None, False, 0, False, False)
            if (li is None): 
                break
            if (li.value is not None): 
                if (li.value.startswith("ОБЛАСТЬ ПРИМЕНЕНИЯ") or li.value == "СОДЕРЖАНИЕ"): 
                    title.end_token = t.previous
                    break
            if (t.is_value("СОДЕРЖАНИЕ", None) and t.is_newline_after): 
                title.end_token = t.previous
                break
            t = li.end_token
            t = t.next0_
        if (empty is not None): 
            empty.end_token = title.end_token
        return title
    ...
    def __createtztitle(t0 : 'Token', doc : 'InstrumentReferent') -> 'FragToken':
        ...
        if (tz is None): 
            return None
        title = FragToken._new1935(t0, tz.end_token, InstrumentKind.HEAD)
        t = t0
        first_pass4091 = True
        while True:
            ...
            if (dt is not None): 
                ...
                if (br is not None and BracketHelper.is_bracket(t.next0_, True)): 
                    nam = FragToken._new1957(br.begin_token, br.end_token, InstrumentKind.NAME, True)
                    title.children.append(nam)
                    t = br.end_token
                    title.end_token = t
                    continue
                if (t.next0_ is not None and t.next0_.is_value("НА", None)): 
                    ...
                    while True:
                        ...
                        if (npt is not None): 
                            tt = npt.end_token
                        t1 = tt
                    nam = FragToken._new1957(t.next0_, t1, InstrumentKind.NAME, True)
                    title.children.append(nam)
                    t = t1
                    title.end_token = t
                    continue
    ...
    @staticmethod
    def _new1935(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrumentKind') -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res.kind = _arg3
        return res
    
    @staticmethod
    def _new1936(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrToken1', _arg4 : bool) -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res._itok = _arg3
    ...
    @staticmethod
    def _new1937(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrumentKind', _arg4 : bool, _arg5 : 'InstrToken1') -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res.kind = _arg3
        res._def_val2 = _arg4
        res._itok = _arg5
    ...
    @staticmethod
    def _new1944(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrToken1') -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res._itok = _arg3
        return res
    
    @staticmethod
    def _new1945(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrumentKind', _arg4 : 'InstrToken1') -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res.kind = _arg3
        res._itok = _arg4
        return res
    ...
    @staticmethod
    def _new1952(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrumentKind', _arg4 : int) -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res.kind = _arg3
        res.number = _arg4
        return res
    ...
    @staticmethod
    def _new1954(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrumentKind', _arg4 : bool) -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res.kind = _arg3
        res._def_val2 = _arg4
        return res
    ...
    @staticmethod
    def _new1957(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrumentKind', _arg4 : bool) -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res.kind = _arg3
        res._def_val = _arg4
        return res
    ...
    @staticmethod
    def _new1969(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrumentKind', _arg4 : object, _arg5 : 'InstrToken1') -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res.kind = _arg3
        res.value = _arg4
        res._itok = _arg5
    ...
    @staticmethod
    def _new1980(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrumentKind', _arg4 : int, _arg5 : bool, _arg6 : typing.List['Referent']) -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res.kind = _arg3
        res.number = _arg4
    ...
    @staticmethod
    def _new1992(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrumentKind', _arg4 : object) -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res.kind = _arg3
        res.value = _arg4
        return res
    ...
    @staticmethod
    def _new1998(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrumentReferent', _arg4 : 'InstrumentKind') -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res._m_doc = _arg3
        res.kind = _arg4
        return res
    ...
    @staticmethod
    def _new2171(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'InstrumentKind', _arg4 : bool, _arg5 : 'InstrToken1') -> 'FragToken':
        res = FragToken(_arg1, _arg2)
        res.kind = _arg3
        res._def_val = _arg4
        res._itok = _arg5
    ...
    @staticmethod
    def _static_ctor():
        FragToken.__m_zapiska_keywords = ["ЗАЯВЛЕНИЕ", "ЗАПИСКА", "РАПОРТ", "ДОКЛАД", "ОТЧЕТ", "ПЛАН"]
