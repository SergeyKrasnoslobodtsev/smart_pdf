...
class InstrToken1(MetaToken):
    ...
    def __str__(self) -> str:
        ...
        if (self.is_num_doubt): 
            print("(?) ", end="", file=res)
        if (self.indents_expired): 
            print("(Indents Expired) ", end="", file=res)
        if (self.is_expired): 
    ...
    def parse(t : 'Token', ignore_directives : bool, cur : 'FragToken'=None, lev : int=0, prev : 'InstrToken1'=None, is_citat : bool=False, max_char : int=0, can_be_table_cell : bool=False, is_in_index : bool=False) -> 'InstrToken1':
        ...
        if (t.is_char_of("({")): 
            ...
            if (fr is not None): 
                edt = InstrToken1._new2145(fr.begin_token, fr.end_token, InstrToken1.Types.EDITIONS)
            else: 
                t2 = InstrToken1._create_edition(t)
                if (t2 is not None): 
                    edt = InstrToken1._new2145(t, t2, InstrToken1.Types.EDITIONS)
            if (edt is not None): 
        ...
        t0 = t
        t00 = None
        res = InstrToken1._new2147(t0, t, True)
        while t is not None: 
        ...
        if (dt is None and (((isinstance(t.get_referent(), PersonReferent)) or (isinstance(t.get_referent(), InstrumentParticipantReferent))))): 
            dt = DecreeToken._new1093(t, t, DecreeToken.ItemType.OWNER)
        ...
        if (t.is_value("ПРИЛОЖЕНИЕ", "ДОДАТОК") and t.morph.case_.is_nominative and t.morph.number == MorphNumber.SINGULAR): 
            if (t.next0_ is not None and ((((t.next0_.is_value("В", None) and not t.next0_.is_newline_after)) or t.next0_.is_char(':')))): 
                pass
            else: 
        ...
        if (t.is_newline_before): 
            ...
            if ((isinstance(t, TextToken)) and t.term == "ПРИМЕЧАНИЯ"): 
                if (t.is_newline_after): 
                    res.typ = InstrToken1.Types.NOTICE
                    return res
                if (t.next0_ is not None and t.next0_.is_char(':') and (isinstance(t.next0_.next0_, NumberToken))): 
                    res.typ = InstrToken1.Types.NOTICE
        ...
        if (t.is_value("КОНСУЛЬТАНТПЛЮС", None) or t.is_value("ГАРАНТ", None) or ((t.is_value("ИНФОРМАЦИЯ", None) and t.is_newline_before))): 
            ...
            if (t1 is not None and ((t1.is_value("ПРИМЕЧАНИЕ", None) or ok))): 
                if (t1.next0_ is not None and t1.next0_.is_char('.')): 
                    t1 = t1.next0_
                re = InstrToken1._new2145(t, t1, InstrToken1.Types.COMMENT)
                hiph = False
                t1 = t1.next0_
                first_pass4104 = True
                while True:
        ...
        if (check_comment > 0 or t.is_value("О", "ПРО")): 
            ...
            while ttt is not None: 
                ...
                ttt = ttt.next0_
            if (ok): 
                cmt = InstrToken1._new2145(t, t1, InstrToken1.Types.COMMENT)
        ...
        if (tt is not None): 
            ...
            if (dt1 is not None and dt1.typ == DecreeToken.ItemType.TYP): 
                ...
                return res
        val = None
        wrapval2156 = RefOutArgWrapper(None)
        tt2 = InstrToken1._check_directive(t, wrapval2156)
        val = wrapval2156.value
        if (tt2 is not None): 
            if (tt2.is_newline_after or ((tt2.next0_ is not None and ((tt2.next0_.is_char_of(":") or ((tt2.next0_.is_char('.') and tt2 != t)))) and ((tt2.next0_.is_newline_after or t.chars.is_all_upper))))): 
                return InstrToken1._new2151(t, (tt2 if tt2.is_newline_after else tt2.next0_), InstrToken1.Types.DIRECTIVE, val)
            ...
            if (tt is not None and tt.is_char_of(".)]")): 
                ...
                while tt is not None: 
                    ...
                    if (tt.is_newline_after): 
                        if (ok): 
                            return InstrToken1._new2145(t, tt, InstrToken1.Types.COMMENT)
                        break
                    tt = tt.next0_
        multi_nums = False
        ...
        while True:
            ...
            if (t.is_newline_before and t != res.begin_token): 
                ...
                if (is_new_line and t.chars.is_letter): 
                    if (not MiscHelper.can_be_start_of_sentence(t)): 
                        nnn = ComplexNumToken.try_parse(t, None, False, False)
                        if (nnn is not None): 
                            pass
                        elif (t.previous is not None and t.previous.is_char_of(":;.]")): 
                            pass
                        elif ((t.previous is not None and t.previous.is_comma and t.previous.previous is not None) and t.previous.previous.is_char(']')): 
                            pass
                        elif ((t.is_value("НЕТ", None) or t.is_value("НЕ", None) or t.is_value("ОТСУТСТВОВАТЬ", None)) or t.is_char('[') or t.is_value("ГДЕ", None)): 
                            pass
            ...
            if (num): 
                ...
                if (len(res.numbers) > 0): 
                    pass
                if (multi_nums and res.min_number is None): 
                    res.numbers.clear()
                    res.typ = InstrToken1.Types.LINE
                ...
                if (res.num_end_token is not None and res.num_end_token.end_char >= t.end_char): 
                    t = res.num_end_token
                    if (res.num_end_token.is_newline_after and res.begin_token == res.num_begin_token): 
                        res.numbers.clear()
                        res.num_begin_token = (None)
            ...
            if ((isinstance(t, TextToken)) and t.chars.is_letter): 
                ...
                if (not t.chars.is_all_upper): 
                    res.all_upper = False
                if (t.is_pure_verb): 
                    if (t.previous is not None and t.previous.is_value("КОТОРЫЙ", None)): 
                        pass
                    elif (t.chars.is_cyrillic_letter): 
            ...
            if (t != res.begin_token and InstrToken1.__is_first_line(t)): 
                break
            tmp = None
            wraptmp2153 = RefOutArgWrapper(None)
            tt2 = InstrToken1._check_directive(t, wraptmp2153)
            tmp = wraptmp2153.value
            if (tt2 is not None): 
        ...
        if (res.typ == InstrToken1.Types.APPENDIX or res.typ_container_rank > 0): 
            ...
            if (tt is not None): 
                ...
                if (ttt is not None): 
                    if (tt.is_value("УТРАТИТЬ", None) and tt.previous is not None and tt.previous.is_char_of("(.-")): 
                        res.value = (None)
                        res.is_expired = True
                    else: 
                        res.typ = InstrToken1.Types.EDITIONS
        ...
        if (res.num_typ != NumberTypes.UNDEFINED and not is_citat): 
            if (res.is_newline_before or is_in_index): 
                ...
                pass
            elif (prev is not None and prev.typ == InstrToken1.Types.NOTICE): 
                pass
            else: 
                bad_number = True
            if (res.num_suffix == "-"): 
        ...
        if (res.typ == InstrToken1.Types.LINE): 
            if (res.num_typ != NumberTypes.UNDEFINED): 
                ...
                for nn in res.numbers: 
                    vv = 0
                    wrapvv2154 = RefOutArgWrapper(0)
                    inoutres2155 = Utils.tryParseInt(nn, wrapvv2154)
                    vv = wrapvv2154.value
                    if (inoutres2155): 
                ...
                if (pt is not None and pt.typ != PartToken.ItemType.PREFIX): 
                    ...
                    if (tt is not None): 
                        ...
                        if (tt.previous is not None): 
                            res.is_expired = tt.previous.is_value("УТРАТИТЬ", "ВТРАТИТИ")
                    if (pt.typ == PartToken.ItemType.INDENTION and res.is_expired and ((len(pt.values) == 1 or len(pt.values) == 2))): 
                        res.indents_expired = True
                        if (len(pt.values) == 1): 
                            res.numbers.append(pt.values[0].value)
                        elif (pt.values[0].int_value > 0 and pt.values[1].int_value > pt.values[0].int_value): 
                            nnn = pt.values[0].int_value
                            while nnn <= pt.values[1].int_value: 
                                res.numbers.append(str(nnn))
                                nnn += 1
                else: 
    ...
    @staticmethod
    def _new2145(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'Types') -> 'InstrToken1':
        res = InstrToken1(_arg1, _arg2)
        res.typ = _arg3
        return res
    
    @staticmethod
    def _new2147(_arg1 : 'Token', _arg2 : 'Token', _arg3 : bool) -> 'InstrToken1':
        res = InstrToken1(_arg1, _arg2)
        res.all_upper = _arg3
        return res
    
    @staticmethod
    def _new2151(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'Types', _arg4 : str) -> 'InstrToken1':
        res = InstrToken1(_arg1, _arg2)
        res.typ = _arg3
        res.value = _arg4
        return res
    ...
    @staticmethod
    def _new2164(_arg1 : 'Token', _arg2 : 'Token', _arg3 : bool, _arg4 : 'Types') -> 'InstrToken1':
        res = InstrToken1(_arg1, _arg2)
