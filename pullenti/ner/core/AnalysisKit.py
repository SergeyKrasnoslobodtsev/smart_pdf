...
class AnalysisKit:
        ...
        if (tokens is not None): 
            ...
            while ii < len(tokens): 
                ...
                if (sofa_.correction_dict is not None): 
                    corw = None
                    wrapcorw625 = RefOutArgWrapper(None)
                    inoutres626 = Utils.tryGetValue(sofa_.correction_dict, mt.term, wrapcorw625)
                    corw = wrapcorw625.value
                    if (inoutres626): 
                        ccc = MorphologyService.process(corw, lang, None)
                        if (ccc is not None and len(ccc) == 1): 
                            tt1 = TextToken._new624(ccc[0], self, tt.begin_char, tt.end_char, tt.term)
                            tt1.chars = tt.chars
                            tt = tt1
    ...
    def correct_word_by_morph(self, tt : 'Token', lang : 'MorphLang') -> 'Token':
        ...
        if (ccc is None or len(ccc) != 1): 
            return None
        tt1 = TextToken._new627(ccc[0], self, tt.begin_char, tt.end_char, tt.chars, tt.term)
    ...
    def embed_token(self, mt : 'MetaToken') -> None:
        ...
        if (isinstance(mt, ReferentToken)): 
            if (mt.referent is not None): 
                mt.referent.add_occurence(TextAnnotation._new628(self.sofa, mt.begin_char, mt.end_char))
    ...
    def get_analyzer_data(self, analyzer : 'Analyzer') -> 'AnalyzerData':
        ...
        if (analyzer is None or analyzer.name is None): 
            return None
        d = None
        wrapd629 = RefOutArgWrapper(None)
        inoutres630 = Utils.tryGetValue(self.__m_datas, analyzer.name, wrapd629)
        d = wrapd629.value
        if (inoutres630): 
            d.kit = self
            return d
    ...
    @staticmethod
    def _new3605(_arg1 : 'Processor', _arg2 : 'ExtOntology') -> 'AnalysisKit':
        res = AnalysisKit()
        res.processor = _arg1
        res.ontology = _arg2
        return res
    ...
    @staticmethod
    def _new3606(_arg1 : 'SourceOfAnalysis', _arg2 : bool, _arg3 : 'MorphLang', _arg4 : EventHandler, _arg5 : 'ExtOntology', _arg6 : 'Processor', _arg7 : bool) -> 'AnalysisKit':
