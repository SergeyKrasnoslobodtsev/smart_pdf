...
class NumberHelper:
        ...
        if (val is not None): 
                ...
                if (str.isdigit(sss[0]) and len(sss) == 3): 
                    ...
                    while True:
                        ...
                        if (ttt.whitespaces_before_count == 1 and ttt.length_char == 3 and str.isdigit(ss[0])): 
                            ii = 0
                            wrapii664 = RefOutArgWrapper(0)
                            inoutres665 = Utils.tryParseInt(ss, wrapii664)
                            ii = wrapii664.value
                            if (not inoutres665): 
                                break
                            val2 += ss
            ...
            for k in range(4):
                if ((isinstance(et.next0_, TextToken)) and et.next0_.chars.is_letter): 
                    ...
                    break
            res = NumberToken._new666(token, et, val, typ, mc)
        ...
        while True:
            ...
            if (isinstance(num.termin.tag, str)): 
                ...
                if (loc_value > 0): 
                    ...
                    et = t
                    continue
            wrapj667 = RefOutArgWrapper(0)
            inoutres668 = Utils.tryParseInt(str(num.termin.tag), wrapj667)
            j = wrapj667.value
            if (not inoutres668): 
                break
    ...
    def try_parse_age(t : 'Token') -> 'NumberToken':
            ...
            if (nt_next is not None): 
                ...
                if (isinstance(t1, TextToken)): 
                    ...
                    if ((v == "ЛЕТ" or v == "ЛЕТИЯ" or v == "ЛЕТИЕ") or v == "РІЧЧЯ" or v == "ЖЫЛЖЫГЫ"): 
                        return NumberToken._new666(t, t1, nt.value, NumberSpellingType.AGE, t1.morph)
        ...
        if (LanguageHelper.ends_with_ex(s, "ЛЕТИЕ", "ЛЕТИЯ", "РІЧЧЯ", None)): 
            ...
            if (term is not None): 
                return NumberToken._new666(tt, tt, str(term.tag), NumberSpellingType.AGE, tt.morph)
        s = tt.lemma
        i = s.find("ЛЕТН")
        if (i < 0): 
            i = s.find("РІЧН")
        if (i > 3): 
            ...
            for i in range(len(ss) - 2, 2, -1):
                ...
                if (len(num0) > 1 and num0[len(num0) - 1] == '0' and len(num1) == 1): 
                    return NumberToken._new666(tt, tt, num0[0:0+len(num0) - 1] + num1, NumberSpellingType.AGE, tt.morph)
            else: i = 2
        return None
    ...
    def __analize_number_tail(tt : 'TextToken', val : str) -> 'MetaToken':
            ...
            if (((s == "<" or s == "(")) and (isinstance(tt.next0_, TextToken))): 
                ...
                if ((s == "TH" or s == "ST" or s == "RD") or s == "ND"): 
                    if (tt.next0_.next0_ is not None and tt.next0_.next0_.is_char_of(">)")): 
                        ...
                        return MetaToken._new674(tt, tt.next0_.next0_, mc)
            return None
        if ((s == "TH" or s == "ST" or s == "RD") or s == "ND"): 
            mc = MorphCollection()
            mc.class0_ = MorphClass.ADJECTIVE
            mc.language = MorphLang.EN
            return MetaToken._new674(tt, tt, mc)
        ...
        if (tt.morph.language.is_ua and mc is None and s == "Ї"): 
            mc = MorphCollection()
            mc.add_item(MorphBaseInfo._new677(MorphClass.ADJECTIVE))
        if (mc is not None): 
            return MetaToken._new674(tt, tt, mc)
        if ((((len(s) < 3) and not tt.is_whitespace_before and tt.previous is not None) and tt.previous.is_hiphen and not tt.previous.is_whitespace_before) and tt.whitespaces_after_count == 1 and s != "А"): 
            if (len(s) == 1 and s != "Й" and not LanguageHelper.is_cyrillic_vowel(s[0])): 
                ...
                return MetaToken._new674(tt, tt, MorphCollection._new679(MorphClass.ADJECTIVE))
        return None
    ...
    def try_parse_real_number(t : 'Token', attrs : 'NumberParseAttr'=NumberParseAttr.NO) -> 'NumberToken':
        ...
        if (t.next0_ is not None and t.next0_.is_value("ЦЕЛЫЙ", "ЦІЛИЙ")): 
            ...
            if ((isinstance(tt1, NumberToken)) or (((isinstance(tt1, TextToken)) and tt1.is_value("НОЛЬ", "НУЛЬ")))): 
                ...
                if (res0.real_value == 0): 
                    ...
                    dd = 0
                    wrapdd683 = RefOutArgWrapper(0)
                    inoutres684 = Utils.tryParseFloat(str0_, wrapdd683)
                    dd = wrapdd683.value
                    if (inoutres684): 
                        pass
                    else: 
                        wrapdd681 = RefOutArgWrapper(0)
                        inoutres682 = Utils.tryParseFloat(str0_.replace('.', ','), wrapdd681)
                        dd = wrapdd681.value
                        if (inoutres682): 
                            pass
                        else: 
                ...
                return res0
        d = 0
        wrapd686 = RefOutArgWrapper(0)
        tt = NumberHelper.__try_parse_float(Utils.asObjectOrNull(t, NumberToken), wrapd686, attrs)
        d = wrapd686.value
        if (tt is None): 
        ...
        if (tt.next0_ is not None and tt.next0_.is_value("ДЕСЯТОК", None)): 
            d *= (10)
            tt = tt.next0_
        return NumberExToken._new685(t0, tt, "", NumberSpellingType.DIGIT, NumberExType.UNDEFINED, d)
    ...
    def get_number_base_info(num_val : str, cla : 'MorphClass') -> 'MorphBaseInfo':
        ...
        Utils.tryParseInt(num_val, wrapn689)
        n = wrapn689.value
        res = MorphBaseInfo._new677(cla)
    ...
    def string_to_double(str0_ : str) -> float:
        ...
        if (str0_ == "NaN"): 
            return math.nan
        wrapres694 = RefOutArgWrapper(0)
        inoutres695 = Utils.tryParseFloat(str0_, wrapres694)
        res = wrapres694.value
        if (inoutres695): 
            return res
        wrapres692 = RefOutArgWrapper(0)
        inoutres693 = Utils.tryParseFloat(str0_.replace('.', ','), wrapres692)
        res = wrapres692.value
        if (inoutres693): 
            return res
        wrapres690 = RefOutArgWrapper(0)
        inoutres691 = Utils.tryParseFloat(str0_.replace(',', '.'), wrapres690)
