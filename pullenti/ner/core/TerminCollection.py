...
class TerminCollection:
    ...
    def __remove_from_tree(self, key : str, t : 'Termin') -> None:
        ...
        while i < len(key): 
            ...
            if (nod.children is None): 
                return
            nn = None
            wrapnn753 = RefOutArgWrapper(None)
            inoutres754 = Utils.tryGetValue(nod.children, ch, wrapnn753)
            nn = wrapnn753.value
            if (not inoutres754): 
                return
            nod = nn
    ...
    def __find_in_tree(self, key : str, lang : 'MorphLang') -> typing.List['Termin']:
        ...
        while i < len(key): 
            ...
            if (nod.children is not None): 
                wrapnn755 = RefOutArgWrapper(None)
                Utils.tryGetValue(nod.children, ch, wrapnn755)
                nn = wrapnn755.value
            if (nn is None): 
                if (ch == (32)): 
    ...
    def __add_to_hash1(self, key : int, t : 'Termin') -> None:
        li = None
        wrapli756 = RefOutArgWrapper(None)
        inoutres757 = Utils.tryGetValue(self.__m_hash1, key, wrapli756)
        li = wrapli756.value
        if (not inoutres757): 
            li = list()
    ...
    def __try_attach_all_(self, token : 'Token', pars : 'TerminParseAttr'=TerminParseAttr.NO, main_root : bool=False) -> typing.List['TerminToken']:
            ...
            if ((((pars) & (TerminParseAttr.TERMONLY))) != (TerminParseAttr.NO)): 
                ...
                while i < tt.invariant_prefix_length_of_morph_vars: 
                    ...
                    if (nod.children is None): 
                        ...
                        break
                    nn = None
                    wrapnn758 = RefOutArgWrapper(None)
                    inoutres759 = Utils.tryGetValue(nod.children, ch, wrapnn758)
                    nn = wrapnn758.value
                    if (not inoutres759): 
                        no_vars = True
                        break
                ...
                if ((no_vars and tt.term0 is not None and tt.term != tt.term0) and tt.invariant_prefix_length_of_morph_vars <= len(tt.term0)): 
                    ...
                    while i < tt.invariant_prefix_length_of_morph_vars: 
                        ...
                        if (nod.children is None): 
                            ...
                            break
                        nn = None
                        wrapnn760 = RefOutArgWrapper(None)
                        inoutres761 = Utils.tryGetValue(nod.children, ch, wrapnn760)
                        nn = wrapnn760.value
                        if (not inoutres761): 
                            no_vars = True
                            break
                        ...
                        i += 1
            if (not no_vars): 
                wrapres766 = RefOutArgWrapper(res)
                inoutres767 = self.__manage_var(token, pars, s, nod, len0, wrapres766)
                res = wrapres766.value
                if (inoutres767): 
                    was_vars = True
                i = 0
                first_pass3862 = True
                while True:
                    ...
                    if (wf.normal_case is None or wf.normal_case == s): 
                        ...
                        if (j < i): 
                            ok = False
                    if (ok): 
                        wrapres762 = RefOutArgWrapper(res)
                        inoutres763 = self.__manage_var(token, pars, wf.normal_case, nod, tt.invariant_prefix_length_of_morph_vars, wrapres762)
                        res = wrapres762.value
                        if (inoutres763): 
                            was_vars = True
                    ...
                    if (j < i): 
                        continue
                    wrapres764 = RefOutArgWrapper(res)
                    inoutres765 = self.__manage_var(token, pars, wf.normal_full, nod, tt.invariant_prefix_length_of_morph_vars, wrapres764)
                    res = wrapres764.value
                    if (inoutres765): 
                        was_vars = True
        elif (isinstance(token, NumberToken)): 
            wrapres768 = RefOutArgWrapper(res)
            inoutres769 = self.__manage_var(token, pars, str(token.value), root, 0, wrapres768)
            res = wrapres768.value
            if (inoutres769): 
                was_vars = True
        else: 
            return None
        if (not was_vars and s is not None and len(s) == 1): 
            vars0_ = [ ]
            wrapvars770 = RefOutArgWrapper(None)
            inoutres771 = Utils.tryGetValue(self.__m_hash1, ord(s[0]), wrapvars770)
            vars0_ = wrapvars770.value
            if (inoutres771): 
                for t in vars0_: 
                    if (not t.lang.is_undefined): 
                        if (not token.morph.language.is_undefined): 
    ...
    def __manage_var(self, token : 'Token', pars : 'TerminParseAttr', v : str, nod : 'CharNode', i0 : int, res : typing.List['TerminToken']) -> bool:
        ...
        while i < len(v): 
            ...
            if (nod.children is None): 
                return False
            nn = None
            wrapnn772 = RefOutArgWrapper(None)
            inoutres773 = Utils.tryGetValue(nod.children, ch, wrapnn772)
            nn = wrapnn772.value
            if (not inoutres773): 
                return False
            nod = nn
    ...
    def find_termins_by_canonic_text(self, text : str) -> typing.List['Termin']:
        if (self.__m_hash_canonic is None): 
            ...
            for t in self.termins: 
                ct = t.canonic_text
                li = [ ]
                wrapli774 = RefOutArgWrapper(None)
                inoutres775 = Utils.tryGetValue(self.__m_hash_canonic, ct, wrapli774)
                li = wrapli774.value
                if (not inoutres775): 
                    li = list()
                ...
                if (not t in li): 
                    li.append(t)
        res = [ ]
        wrapres776 = RefOutArgWrapper(None)
        inoutres777 = Utils.tryGetValue(self.__m_hash_canonic, text, wrapres776)
        res = wrapres776.value
        if (not inoutres777): 
            return None
        else: 
            return res