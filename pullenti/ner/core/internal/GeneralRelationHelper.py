...
class GeneralRelationHelper:
    ...
    class Node:
        ...
        @staticmethod
        def _new547(_arg1 : 'Referent', _arg2 : 'AnalyzerData') -> 'Node':
    ...
    def refresh_generals(proc : 'Processor', kit : 'AnalysisKit') -> None:
        ...
        for a in proc.analyzers: 
            ...
            for r in ad.referents: 
                nod = GeneralRelationHelper.Node._new547(r, ad)
                all_refs.append(nod)
                r.tag = (nod)
                si = None
                wrapsi550 = RefOutArgWrapper(None)
                inoutres551 = Utils.tryGetValue(all0_, a.name, wrapsi550)
                si = wrapsi550.value
                if (not inoutres551): 
                    si = dict()
                    all0_[a.name] = si
                ...
                for s in strs: 
                    if (s is None): 
                        continue
                    li = [ ]
                    wrapli548 = RefOutArgWrapper(None)
                    inoutres549 = Utils.tryGetValue(si, s, wrapli548)
                    li = wrapli548.value
                    if (not inoutres549): 
                        li = list()
                        si[s] = li
