...
class _NounPraseHelperInt:
    ...
    def __try_parse_ru(first : 'Token', typ : 'NounPhraseParseAttr', max_char_pos : int, def_noun : 'NounPhraseItem'=None) -> 'NounPhraseToken':
        ...
        if (noun is None): 
            return None
        res = NounPhraseToken._new620(first, noun.end_token, prep)
        if (adverbs is not None): 
            for a in adverbs: 
        ...
        if (stat is not None): 
            for adj in items: 
                if (adj.morph.items_count > 1): 
                    ...
                    i1 = 0
                    i2 = 0
                    wrapi1622 = RefOutArgWrapper(0)
                    Utils.tryGetValue(stat, l1, wrapi1622)
                    i1 = wrapi1622.value
                    wrapi2621 = RefOutArgWrapper(0)
                    Utils.tryGetValue(stat, l2, wrapi2621)
                    i2 = wrapi2621.value
                    if (i1 < i2): 
    ...
    def __try_parse_en(first : 'Token', typ : 'NounPhraseParseAttr', max_char_pos : int) -> 'NounPhraseToken':
        ...
        if (res.morph.items_count == 0 and has_article): 
            res.morph.add_item(MorphBaseInfo._new273(MorphClass.NOUN, MorphNumber.SINGULAR))
        i = 0
        while i < (len(items) - 1): 
            res.adjectives.append(items[i])
            i += 1
