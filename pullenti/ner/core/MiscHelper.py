...
class MiscHelper:
                            ...
                            if (it.gender == npt.morph.gender or ((it.gender) & (npt.morph.gender)) != (MorphGender.UNDEFINED)): 
                                if (not ((it.case_) & npt.morph.case_).is_undefined): 
                                    if (it.number == npt.morph.number or ((it.number) & (npt.morph.number)) != (MorphNumber.UNDEFINED)): 
                                        ...
                                        if (isinstance(it, MorphWordForm)): 
                                            var = it.normal_case
                                        bi = MorphBaseInfo._new646(MorphClass.ADJECTIVE, npt.morph.gender, npt.morph.number, npt.morph.language)
                                        try: 
                                            var = MorphologyService.get_wordform(var, bi)
                                        except Exception as ex647: 
                                            pass
                                        if (var is not None): 
                ...
                if (isinstance(t, NumberToken)): 
                    ...
                    if (not t.chars.is_letter): 
                        ...
                        if ((t.next0_ is not None and t.next0_.is_hiphen and t.is_value("ГЕНЕРАЛ", None)) or t.is_value("КАПИТАН", None)): 
                            ...
                            mbi = MorphBaseInfo._new649(npt.morph.gender, cas, MorphNumber.SINGULAR)
                            if (num != MorphNumber.UNDEFINED): 
                                mbi.number = num
                            wcas = None
                            try: 
                                wcas = MorphologyService.get_wordform(word, mbi)
                            except Exception as ex650: 
                                pass
                            if (wcas is not None): 
                                word = wcas
    ...
    def get_text_morph_var_by_case_and_number_ex(str0_ : str, cas : 'MorphCase'=None, num : 'MorphNumber'=MorphNumber.SINGULAR, num_val : str=None) -> str:
        ...
        ar = None
        try: 
            ar = ProcessorService.get_empty_processor().process(SourceOfAnalysis._new651(str0_, False), None, None)
        except Exception as ex: 
            pass
        if (ar is None or ar.first_token is None): 
            return str0_
        ...
        adj_bi = MorphBaseInfo._new652((MorphClass.ADJECTIVE) | MorphClass.NOUN, cas, num, npt.morph.gender)
        noun_bi = MorphBaseInfo._new653(npt.noun.morph.class0_, cas, num)
        ...
        if (not Utils.isNullOrEmpty(num_val) and num == MorphNumber.UNDEFINED): 
            ...
            n = 0
            wrapn654 = RefOutArgWrapper(0)
            Utils.tryParseInt(num_val, wrapn654)
            n = wrapn654.value
    ...
    def get_pure_referent_token(rt : 'ReferentToken') -> 'MetaToken':
        ...
        if (t0 is None): 
            t0 = rt.begin_token
            t1 = rt.end_token
        return MetaToken._new657(t0, t1, MiscHelper.get_text_value(t0, t1, Utils.valToEnum((GetTextAttr.KEEPQUOTES) | (GetTextAttr.KEEPREGISTER), GetTextAttr)))
