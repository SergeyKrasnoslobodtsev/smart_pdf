...
class OrganizationReferent(Referent):
    ...
    def __correct_name(self, name : str, num : int) -> str:
        ...
        if (str.isdigit(name[0]) and name.find(' ') > 0): 
            i = 0
            wrapi3118 = RefOutArgWrapper(0)
            inoutres3119 = Utils.tryParseInt(name[0:0+name.find(' ')], wrapi3118)
            i = wrapi3118.value
            if (inoutres3119): 
                if (i > 1): 
                    num.value = i
            ...
            if (i >= 0 and name[i] == '.'): 
                ...
                inoutres3120 = Utils.tryParseInt(name[i + 1:], num)
                if (i > 0 and inoutres3120 and num.value > 0): 
                    if (i < 1): 
                        return None
    ...
    def __correct_type(self, name : str) -> str:
        ...
        first_pass4200 = True
        while True:
            if first_pass4200: first_pass4200 = False
            else: i += 1
    ...
    def add_name(self, name : str, remove_long_gov_names : bool=True, t : 'Token'=None) -> None:
        num = 0
        wrapnum3121 = RefOutArgWrapper(0)
        s = self.__correct_name(name, wrapnum3121)
        num = wrapnum3121.value
        if (s == "ЗАКАЗЧИК"): 
            pass
        if (s is None): 
    ...
    def add_name_str(self, name : str, typ : 'OrgItemTypeToken', cou : int=1) -> None:
        ...
        if (name is None): 
            ...
            if (typ.name is not None and Utils.compareStrings(typ.name, typ.typ, True) != 0 and ((len(typ.name) > len(typ.typ) or self.find_slot(OrganizationReferent.ATTR_NAME, None, True) is None))): 
                num = 0
                wrapnum3122 = RefOutArgWrapper(0)
                s = self.__correct_name(typ.name, wrapnum3122)
                num = wrapnum3122.value
            ...
            if (typ is None or typ.is_not_typ): 
                ...
                if (typ.name is not None): 
                    num = 0
                    wrapnum3123 = RefOutArgWrapper(0)
                    ss = self.__correct_name(typ.name, wrapnum3123)
                    num = wrapnum3123.value
                    if (ss is not None): 
    ...
    def __get_sorted_types(self, for_ontos : bool) -> typing.List[str]:
        ...
        first_pass4201 = True
        while True:
            if first_pass4201: first_pass4201 = False
            else: i += 1
            ...
            if (((eq_number or eq_eponyms)) and ((self.find_slot(OrganizationReferent.ATTR_NAME, None, True) is None or org0_.find_slot(OrganizationReferent.ATTR_NAME, None, True) is None))): 
                ...
                for v in self._name_vars.items(): 
                    if (typ == ReferentsEqualType.DIFFERENTTEXTS and v[1]): 
                        continue
                    b = False
                    wrapb3125 = RefOutArgWrapper(False)
                    inoutres3126 = Utils.tryGetValue(org0_._name_vars, v[0], wrapb3125)
                    b = wrapb3125.value
                    if (not inoutres3126): 
                        continue
