...
class DecreeHelper:
    ...
    def parse_date_time(str0_ : str) -> datetime.datetime:
        ...
        try: 
            ...
            y = 0
            wrapy1081 = RefOutArgWrapper(0)
            inoutres1082 = Utils.tryParseInt(prts[0], wrapy1081)
            y = wrapy1081.value
            if (not inoutres1082): 
                return None
            mon = 0
            day = 0
            if (len(prts) > 1): 
                wrapmon1079 = RefOutArgWrapper(0)
                inoutres1080 = Utils.tryParseInt(prts[1], wrapmon1079)
                mon = wrapmon1079.value
                if (inoutres1080): 
                    if (len(prts) > 2): 
                        wrapday1078 = RefOutArgWrapper(0)
                        Utils.tryParseInt(prts[2], wrapday1078)
                        day = wrapday1078.value
            if (mon <= 0): 
                mon = 1
            if (day <= 0): 
                day = 1
    ...
    def try_create_canonic_decree_ref_uri(t : 'Token') -> 'CanonicDecreeRefUri':
        ...
        if (dr is not None): 
            if (dr.kind == DecreeKind.PUBLISHER): 
                return None
            res = CanonicDecreeRefUri._new1083(t.kit.sofa.text, dr, t.begin_char, t.end_char)
            ...
            if (rt.begin_token.is_char('(') and rt.end_token.is_char(')')): 
                res = CanonicDecreeRefUri._new1083(t.kit.sofa.text, dr, rt.begin_token.next0_.begin_char, rt.end_token.previous.end_char)
                return res
        ...
        res = CanonicDecreeRefUri._new1085(t.kit.sofa.text, dpr, t.begin_char, t1.end_char, has_diap)
    ...
    def check_nds(t : 'Token', nds : float=18, nds_mustbe_money : bool=False) -> 'MetaToken':
                ...
                if (ne is not None and ne.ex_typ == NumberExType.PERCENT): 
                    if (math.fabs(ne.real_value - nds) > 0.0001): 
                        ...
                        if (has_nds): 
                            ok = True
                        if (ok): 
                            return MetaToken._new657(tt, ne.end_token, "Размер НДС должен быть {0}%, а не {1}%".format(nds, ne.real_value))
                    ndst1 = ne.end_token
                    tt = ndst1
                    has_nds_perc = True
                    continue
        ...
        if (m1 is None): 
            if (nds_mustbe_money): 
                return MetaToken._new657(ndst0, ndst1, "Размер НДС должен быть в денежном выражении")
            return None
        if (has_all): 
            return None
        ...
        if (delta > 0.011): 
            if ((delta < 1) and m1.rest == 0 and m.rest == 0): 
                ...
                mr = MoneyReferent._new1088(m1.currency, must_be)
                return MetaToken._new657(t, tt, "Размер НДС должен быть {0}, а не {1}".format(DecreeHelper.__out_money(mr), DecreeHelper.__out_money(m1)))
        if (incl): 
            return None
        m2 = None
        has_all = False
        ...
        if (m2 is not None and has_all): 
            ...
            if (delta > 0.01): 
                ...
                return MetaToken._new657(t, tt, err)
        return None
    
    @staticmethod
    def remove_psevdo_decreeparts(kit : 'AnalysisKit') -> int:
        if ("RemovePsevdoDecreeparts" in kit.misc_data): 
            return kit.misc_data["RemovePsevdoDecreeparts"]
        cou = 0
        t = kit.first_token
        first_pass3898 = True
        while True:
            ...
            if (t.is_newline_before and t.is_newline_after): 
                if (isinstance(t.get_referent(), DecreePartReferent)): 
                    tt = t.next0_
                    if (tt is None): 
                        continue
                    if (tt.is_value("ВВЕСТИ", None) or tt.is_comma_and): 
                        continue
                    t = kit.debed_token(t)
                    cou += 1
                    continue
            if (t.is_newline_before and (isinstance(t.get_referent(), DecreeChangeReferent))): 
                dch = Utils.asObjectOrNull(t.get_referent(), DecreeChangeReferent)
                if (dch.kind == DecreeChangeKind.CONTAINER): 
                    if ((t.begin_token.is_value("ИЗМЕНЕНИЕ", None) or t.begin_token.is_value("ПЕРЕЧЕНЬ", None) or t.begin_token.is_value("О", None)) or t.begin_token.is_value("В", None) or (isinstance(t.begin_token, ReferentToken))): 
                        t = t.kit.debed_token(t)
                        cou += 1
                        continue
        if ((isinstance(kit.first_token, ReferentToken)) and kit.first_token.get_referent().type_name == "ORGANIZATION" and kit.first_token.is_newline_after): 
            if ((isinstance(kit.first_token.next0_, ReferentToken)) and (isinstance(kit.first_token.next0_.get_referent(), DecreeReferent))): 
                kit.debed_token(kit.first_token.next0_)
                cou += 1
        kit.misc_data["RemovePsevdoDecreeparts"] = cou
        return cou