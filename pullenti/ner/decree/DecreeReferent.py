...
class DecreeReferent(Referent):
    ...
    ATTR_OWNER = "OWNER"
    """ Имя атрибута - владелец (тот, кто утвердил данный НПА) """
    ...
    def to_string_ex(self, short_variant : bool, lang : 'MorphLang'=None, lev : int=0) -> str:
        ...
        out_part = False
        nam = self.name
        if (nam is not None and self.owner is not None): 
            nam = self.get_string_value(DecreeReferent.ATTR_NAME)
            print("\"{0}\"".format(MiscHelper.convert_first_char_upper_and_other_lower(nam)), end="", file=res, flush=True)
        ...
        if (self.owner is not None and nam is not None): 
            if (self.owner.owner is not None): 
                print(", утв. ERROR (циклическая зависимость)".format(), end="", file=res, flush=True)
            else: 
                print(", утв. {0}".format(self.owner.to_string_ex(True, None, 0)), end="", file=res, flush=True)
        elif (not short_variant): 
    ...
    def __all_years(self) -> typing.List[int]:
        ...
        for s in self.slots: 
            if (s.type_name == DecreeReferent.ATTR_DATE): 
                ...
                if (i == 4): 
                    str0_ = str0_[0:0+4]
                wrapi1416 = RefOutArgWrapper(0)
                inoutres1417 = Utils.tryParseInt(str0_, wrapi1416)
                i = wrapi1416.value
                if (inoutres1417): 
                    res.append(i)
        return res
    ...
    def owner(self) -> 'DecreeReferent':
        """ НПА, утвердивший данный НПА """
        res = Utils.asObjectOrNull(self.get_slot_value(DecreeReferent.ATTR_OWNER), DecreeReferent)
        if (res is None): 
            return None
        return res
    @owner.setter
    def owner(self, value) -> 'DecreeReferent':
        if (value is not None): 
            if (value.owner is not None or value == self): 
                return value
        self.add_slot(DecreeReferent.ATTR_OWNER, value, True, 0)
        return value
    ...
    def __can_be_equals(self, obj : 'Referent', typ_ : 'ReferentsEqualType', ignore_geo : bool) -> bool:
        ...
        if (self.owner is not None and dr.owner is not None): 
            if (not self.owner.can_be_equals(dr.owner, typ_)): 
                return False
        ...
        if (self.find_slot(DecreeReferent.ATTR_NAME, None, True) is not None and dr.find_slot(DecreeReferent.ATTR_NAME, None, True) is not None): 
            ...
            if (self.is_law and date_not_eq): 
                return False
            if (self.owner is not None or dr.owner is not None): 
                if (self.owner is None or dr.owner is None): 
                    return False
        if (date_not_eq): 
            return False
        ty = self.typ
    ...
    def check_typename(self, type_name_ : str) -> bool:
        if (type_name_ == self.typ or type_name_ == self.typ0): 
            return True
        for s in self.slots: 
            if (s.type_name == DecreeReferent.ATTR_NAME): 
                nam = Utils.ifNotNull(Utils.asObjectOrNull(s.value, str), "")
                ii = nam.find(' ')
                if (ii > 0): 
                    nam = nam[0:0+ii]
                if (MiscHelper.can_be_equals(nam, type_name_, True, True, False)): 
                    return True
        return False
    ...
    def _check_correction(self, noun_is_doubtful : bool) -> bool:
        ...
        if (ty == DecreeKind.STANDARD): 
            if (self.number is not None and ((len(self.number) >= 4 or self.number.find('.') > 0))): 
                return True
            if (self.find_slot(DecreeReferent.ATTR_NAME, None, True) is None): 
                if (self.number is not None and ((self.typ == "СТАНДАРТ ОРГАНИЗАЦИИ" or self.typ == "ISO"))): 
                    pass
