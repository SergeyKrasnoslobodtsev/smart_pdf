...
class DecreeAnalyzer(Analyzer):
    ...
    class ThisDecree(MetaToken):
        ...
        def try_attach_back(t : 'Token', base_typ : 'DecreeToken') -> 'ThisDecree':
            ...
            if (ukaz is not None): 
                if (base_typ is not None and base_typ.value is not None and (("ДОГОВОР" in base_typ.value or "ДОГОВІР" in base_typ.value))): 
                    return DecreeAnalyzer.ThisDecree._new1408(ukaz, ukaz, True, base_typ.value)
            return None
        ...
        def try_attach(dtok : 'PartToken', base_typ : 'DecreeToken') -> 'ThisDecree':
            ...
            if (not (isinstance(tt, TextToken))): 
                if ((isinstance(tt, ReferentToken)) and (isinstance(tt.get_referent(), DecreeReferent))): 
                    return DecreeAnalyzer.ThisDecree._new1409(t, tt, Utils.asObjectOrNull(tt.get_referent(), DecreeReferent))
                return None
        ...
        @staticmethod
        def _new1408(_arg1 : 'Token', _arg2 : 'Token', _arg3 : bool, _arg4 : str) -> 'ThisDecree':
        ...
        @staticmethod
        def _new1409(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'DecreeReferent') -> 'ThisDecree':
            res = DecreeAnalyzer.ThisDecree(_arg1, _arg2)
            res.real = _arg3
            return res
    ...
    def _try_attach(dts : typing.List['DecreeToken'], base_typ : 'DecreeToken', ad : 'AnalyzerData') -> typing.List['ReferentToken']:
        ...
        if (res is None and len(dts) == 1 and dts[0].typ_ref_back): 
            ...
            while True:
                ...
                if (dr.check_typename(dts[0].value)): 
                    ...
                    continue
                res = list()
                res.append(ReferentToken(dr, dts[0].begin_token, dts[0].end_token))
                break
        ad.level -= 1
        return res
    ...
    def _try_attach_approved(t : 'Token', ad : 'AnalyzerData') -> 'ReferentToken':
        ...
        while True:
            ...
            if ((isinstance(tt, TextToken)) and not tt.chars.is_letter and tt.length_char == 1): 
                if (not BracketHelper.is_bracket(tt, False) and not tt.is_char_of(",.:;№\\/") and not tt.is_hiphen): 
                    break
            if (tt.is_value2("ЯВЛЯЮЩИЙСЯ", "ПРИЛОЖЕНИЕМ")): 
                keyword_after = True
                break
            ...
            if (tt.is_char('(')): 
                ...
                if (mt is not None): 
                    ...
                    if (tt.next0_ is not None and tt.next0_.is_comma_and and (t.end_char < tt0.previous.begin_char)): 
                        next0_ = DecreeAnalyzer._try_attach_approved(tt.next0_.next0_, ad)
                        if (next0_ is not None): 
                            ...
                            return ReferentToken._new950(dec0, t, tt, alias)
                    continue
        ...
        if (((tt is not None or ((last_tok is not None and (cou < 30))))) and key_tok is not None and ((not t.chars.is_all_lower or ((t.previous is not None and t.previous.is_value("В", None)))))): 
            if (t.previous is not None and t.previous.is_value("ТЕКСТ", None)): 
                return None
            if (t.chars.is_all_lower and not keyword_after): 
                return None
            cou = 500
            if (tt is None): 
    ...
    def _try_attach_approved0(t : 'Token', ad : 'AnalyzerData', must_be_comma : bool=True, name_in_brackets : bool=False) -> 'ReferentToken':
        ...
        olev = None
        lev = 0
        wrapolev1394 = RefOutArgWrapper(None)
        inoutres1395 = Utils.tryGetValue(kit.misc_data, "dovr", wrapolev1394)
        olev = wrapolev1394.value
        if (not inoutres1395): 
            lev = 1
    ...
    def __try_attach_pulishers(self, dts : typing.List['DecreeToken']) -> typing.List['ReferentToken']:
        ...
        while True:
            ...
            if (pt is not None): 
                ...
                if (pub_part is None): 
                    pub_part = DecreePartReferent._new1396(pub)
            ...
            if (isinstance(t, NumberToken)): 
                ...
                if (rt is not None): 
                    date_ = DecreeToken._new1093(rt.begin_token, rt.end_token, DecreeToken.ItemType.DATE)
                    date_.ref = rt
                    pub0 = (None)
                    pub_part0 = (None)
                    if (t0 is None): 
    ...
    def process(self, kit : 'AnalysisKit') -> None:
        ...
        while True:
            ...
            if (t.is_value2("СЛЕДУЮЩЕГО", "СОДЕРЖАНИЯ") or t.is_value2("СЛЕДУЮЩЕЙ", "РЕДАКЦИИ") or ((t.is_value2("НОВОЙ", "РЕДАКЦИИ") and t.next0_.next0_ is not None and t.next0_.next0_.is_char(':')))): 
                ...
                if (tt is not None and tt.is_char(':')): 
                    tt = tt.next0_
                if (BracketHelper.can_be_start_of_sequence(tt, True, False)): 
                    txt = DecreeChangeToken._try_parse_text(tt, False, False)
                    if (txt is not None): 
                        t = txt.end_token
                        continue
        ...
        while True:
            ...
            if (ok1): 
                ...
                dcht = DecreeChangeToken.try_attach(t, (None if root_change is None else Utils.asObjectOrNull(root_change.referent, DecreeChangeReferent)), False, change_stack, False, False)
                if (dcht is not None): 
            ...
            if (dcht is not None and ((dcht.is_start or dcht.change_val is not None)) and not dcht.ignorable): 
                if (dcht.typ == DecreeChangeTokenTyp.STARTMULTU): 
                    ...
                    dcht1 = DecreeChangeToken.try_attach(dcht.end_token.next0_, (None if root_change is None else Utils.asObjectOrNull(root_change.referent, DecreeChangeReferent)), False, change_stack, False, False)
                    ...
                    if (not ok): 
                        ...
                        while True:
                            ...
                            if (not (tt2 is not None)): break
                            next0_ = DecreeChangeToken.try_attach(tt2, None, True, None, False, False)
                            if (next0_ is not None): 
                ...
                if (has_start_change <= 0 and expire_regime == 0): 
                    ...
                    if (dcht.decree is not None): 
                        if (len(change_stack) > 0 and change_stack[len(change_stack) - 1] != dcht.decree): 
                            root_change = (None)
                        change_stack.clear()
                ...
                dts = PartToken.try_attach_list(t, False, 40)
                if (dcht is None and t.is_newline_before and not t.is_char('(')): 
                    expire_regime = 0
                    has_start_change -= 1
            if (dts is not None and len(dts) > 0 and root_change is not None): 
                if (dts[0].begin_char < root_change.begin_char): 
                    dts = (None)
            if (dts is not None and len(dts) == 1 and root_change is None): 
                if ((dts[0].typ == PartToken.ItemType.FORM or dts[0].typ == PartToken.ItemType.FORMULA or dts[0].typ == PartToken.ItemType.NAME) or dts[0].typ == PartToken.ItemType.TABLE): 
                    dts = (None)
            rts = DecreeAnalyzer._try_attach_parts(dts, base_typ, (change_stack[0] if has_start_change > 0 and len(change_stack) > 0 else None), root_change is not None)
            if (rts is not None): 
                pass
            dprs = None
            diaps = None
                    ...
                    while True:
                        ...
                        if (len(rts) == 1 and len(dpr_list) == 1): 
                            ...
                            if (dcht is not None and dcht.parts == dts): 
                                dcht.real_part = dr
                            if (len(change_stack) == 1 or ((dcht is not None and dcht.typ == DecreeChangeTokenTyp.SINGLE))): 
                                change_stack.insert(0, dr)
                        if (dprs is None): 
                            dprs = list()
                        dprs.append(dr)
                        rt = None
                        if (i == 0): 
                            rt = ReferentToken._new948(dr, kp.begin_token, kp.end_token, kp.morph)
                        else: 
            ...
            if (dcht is not None and ((has_start_change > 0 or expire_regime > 0)) and not dcht.ignorable): 
                ...
                if ((dcht.typ == DecreeChangeTokenTyp.SINGLE and dcht.decree is not None and dcht.end_token.is_char(':')) and dcht.act_kind == DecreeChangeKind.UNDEFINED): 
                    if (root_change is None or len(change_stack) == 0): 
                        dcht.typ = DecreeChangeTokenTyp.STARTMULTU
                    elif (change_stack[len(change_stack) - 1] != dcht.decree): 
                        dcht.typ = DecreeChangeTokenTyp.STARTMULTU
                if (dcht.typ == DecreeChangeTokenTyp.STARTMULTU and ((root_change_ques is None or root_change_ques.decree != dcht.decree))): 
                    ...
                    if (((len(change_stack) > 0 or dcht.decree is not None)) and expire_regime == 0): 
                        rch = Utils.asObjectOrNull(ad.register_referent(DecreeChangeReferent._new1400(DecreeChangeKind.CONTAINER)), DecreeChangeReferent)
                ...
                if (dprs is not None and len(dprs) > 0): 
                    ...
                    if (chrt is None and expire_regime > 0): 
                        chrt = list()
                        dcr = DecreeChangeReferent._new1400((DecreeChangeKind.EXPIRE if expire_regime == 1 else DecreeChangeKind.SUSPEND))
                ...
                if (chrt is None and ((expire_regime > 0 or dcht.act_kind == DecreeChangeKind.EXPIRE or dcht.act_kind == DecreeChangeKind.SUSPEND)) and dprs is None): 
                    chrt = list()
                    if (dcht.decree is not None): 
                        dcr = DecreeChangeReferent._new1400(DecreeChangeKind.EXPIRE)
                    ...
                    while True:
                        ...
                        if (isinstance(tt.get_referent(), DecreeReferent)): 
                            dcr = DecreeChangeReferent._new1400((DecreeChangeKind.EXPIRE if expire_regime == 1 or dcht.act_kind == DecreeChangeKind.EXPIRE else DecreeChangeKind.SUSPEND))
    ...
    def _check_alias_after(t : 'Token') -> 'MetaToken':
        ...
        if ((isinstance(t.get_referent(), DecreeReferent)) and t.next0_ is not None and t.next0_.is_char(')')): 
            return MetaToken._new657(t, t.next0_, t.get_referent())
        t1 = None
        tt = t
        while tt is not None: 
            if (tt.is_newline_before): 
        ...
        while i < len(res): 
            ...
            while et.begin_char > res[i].begin_char:
                if (et.is_char(',') or et.morph.class0_.is_conjunction or et.is_hiphen): 
                    ...
                    break
            res1.append(MetaToken._new1407(res[i].begin_token, (et if et.begin_char >= res[i].begin_char else res[i].begin_token), li, res[i].morph))
            i = (j - 1)
            i += 1
        return res1