...
class PersonHelper:
    ...
    def _create_referent_token(p : 'PersonReferent', begin : 'Token', end : 'Token', morph_ : 'MorphCollection', attrs : typing.List['PersonAttrToken'], for_attribute : bool, after_be_predicate : bool) -> 'ReferentToken':
            ...
            if (ttt.is_value("ИМЕНИ", "ІМЕНІ")): 
                ...
                if (ttt.whitespaces_after_count < 3): 
                    if (ttt.is_value("ИМ", "ІМ")): 
                        for_attribute = True
        if (for_attribute): 
            return ReferentToken._new3212(p, begin, end, morph_, p._m_person_identity_typ)
        ...
        first_pass4214 = True
        while True:
            if first_pass4214: first_pass4214 = False
            else: t = t.next0_
            ...
            if (t.is_char('(') and t == end.next0_): 
                ...
                if ((pit1 is not None and t.chars.is_capital_upper and pit1.end_token.next0_ is not None) and (isinstance(t, TextToken)) and pit1.end_token.next0_.is_char(')')): 
                    if (pit1.lastname is not None): 
                        inf = MorphBaseInfo._new3204(MorphCase.NOMINATIVE)
                        if (p.is_male): 
        ...
        first_pass4215 = True
        while True:
            if first_pass4215: first_pass4215 = False
            else: t = t.next0_
            if (not (t is not None)): break
            if (t.is_table_control_char): 
                break
            ...
            if (r is not None and r.type_name == "ORGANIZATION"): 
                ...
                if (not exist): 
                    ...
                    pat.prop_ref = PersonPropertyReferent._new3174("сотрудник")
    ...
    def create_nickname(pr : 'PersonReferent', t : 'Token') -> 'Token':
        ...
        first_pass4216 = True
        while True:
            if first_pass4216: first_pass4216 = False
            else: t = t.next0_
            if (not (t is not None)): break
        ...
        if (BracketHelper.is_bracket(t, True)): 
            ...
            if (br is not None): 
                ...
                if (ni is not None): 
                    ...
                    first_pass4217 = True
                    while True:
                        if first_pass4217: first_pass4217 = False
                        else: tt = tt.next0_
            ...
            first_pass4218 = True
            while True:
                if first_pass4218: first_pass4218 = False
                else: t = t.next0_
                if (not (t is not None)): break
                if (t.is_comma_and): 
                    continue
