...
class ShortNameHelper:
    ...
    class ShortnameVar:
        ...
        @staticmethod
        def _new3341(_arg1 : str, _arg2 : 'MorphGender') -> 'ShortnameVar':
    ...
    def get_names_for_shortname(shortname : str) -> typing.List['ShortnameVar']:
        res = [ ]
        wrapres3339 = RefOutArgWrapper(None)
        inoutres3340 = Utils.tryGetValue(ShortNameHelper.M_SHORTS_NAMES, shortname, wrapres3339)
        res = wrapres3339.value
        if (not inoutres3340): 
            return None
        else: 
            return res
        ...
        if (obj is not None): 
            ...
            while t is not None: 
                if (t.is_newline_before): 
                    ...
                    for s in shos: 
                        li = None
                        wrapli3342 = RefOutArgWrapper(None)
                        inoutres3343 = Utils.tryGetValue(ShortNameHelper.M_SHORTS_NAMES, s, wrapli3342)
                        li = wrapli3342.value
                        if (not inoutres3343): 
                            li = list()
                            ShortNameHelper.M_SHORTS_NAMES[s] = li
                        li.append(ShortNameHelper.ShortnameVar._new3341(nam, g))
                    if (t is None): 
                        break
                    t = t.previous
                t = t.next0_
