...
class PersonMorphCollection:
    ...
    class PersonMorphVariant:
        ...
        @staticmethod
        def _new3222(_arg1 : str) -> 'PersonMorphVariant':
            res = PersonMorphCollection.PersonMorphVariant()
            res.value = _arg1
            return res
        
        @staticmethod
        def _new3333(_arg1 : str, _arg2 : 'MorphGender', _arg3 : str) -> 'PersonMorphVariant':
            res = PersonMorphCollection.PersonMorphVariant()
            res.value = _arg1
            res.gender = _arg2
        ...
        if (gen == MorphGender.MASCULINE or gen == MorphGender.FEMINIE): 
            for it in self.items: 
                if (it.value == val and it.gender == gen): 
                    return
            self.items.append(PersonMorphCollection.PersonMorphVariant._new3333(val, gen, shortval))
            if (add_other_gender_var): 
                g0 = (MorphGender.MASCULINE if gen == MorphGender.FEMINIE else MorphGender.FEMINIE)
                v = None
                try: 
                    v = MorphologyService.get_wordform(val, MorphBaseInfo._new275(MorphClass._new3313(True), g0))
                except Exception as ex3336: 
                    pass
                if (v is not None): 
                    self.items.append(PersonMorphCollection.PersonMorphVariant._new3333(v, g0, shortval))
        else: 
