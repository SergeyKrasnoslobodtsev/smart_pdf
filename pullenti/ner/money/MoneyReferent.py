...
class MoneyReferent(Referent):
    ...
    def value(self) -> float:
        ...
        if (val is None): 
            return 0
        v = 0
        wrapv2383 = RefOutArgWrapper(0)
        inoutres2384 = Utils.tryParseFloat(val, wrapv2383)
        v = wrapv2383.value
        if (not inoutres2384): 
            return 0
        return v
    ...
    def alt_value(self) -> float:
        ...
        if (val is None): 
            return None
        v = 0
        wrapv2385 = RefOutArgWrapper(0)
        inoutres2386 = Utils.tryParseFloat(val, wrapv2385)
        v = wrapv2385.value
        if (not inoutres2386): 
            return None
        return v
    ...
    def rest(self) -> int:
        ...
        if (val is None): 
            return 0
        v = 0
        wrapv2387 = RefOutArgWrapper(0)
        inoutres2388 = Utils.tryParseInt(val, wrapv2387)
        v = wrapv2387.value
        if (not inoutres2388): 
            return 0
        return v
    ...
    def alt_rest(self) -> int:
        ...
        if (val is None): 
            return None
        v = 0
        wrapv2389 = RefOutArgWrapper(0)
        inoutres2390 = Utils.tryParseInt(val, wrapv2389)
        v = wrapv2389.value
        if (not inoutres2390): 
            return None
        return v
    ...
    @staticmethod
    def _new1088(_arg1 : str, _arg2 : float) -> 'MoneyReferent':
        res = MoneyReferent()
        res.currency = _arg1
