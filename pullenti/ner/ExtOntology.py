...
class ExtOntology:
    ...
    def add(self, ext_id : object, type_name : str, definition_ : str) -> 'ExtOntologyItem':
        ...
        if (rs is None): 
            return None
        self.__m_hash = (None)
        res = ExtOntologyItem._new3573(ext_id, rs[0], type_name)
        if (len(rs) > 1): 
            del rs[0]
            res._refs = rs
    ...
    def add_referent(self, ext_id : object, referent : 'Referent') -> 'ExtOntologyItem':
        ...
        if (referent is None): 
            return None
        self.__m_hash = (None)
        res = ExtOntologyItem._new3573(ext_id, referent, referent.type_name)
        self.items.append(res)
        return res
    
    def __create_referent(self, type_name : str, definition_ : str) -> typing.List['Referent']:
        analyzer = None
        wrapanalyzer3575 = RefOutArgWrapper(None)
        inoutres3576 = Utils.tryGetValue(self.__m_anal_by_type, type_name, wrapanalyzer3575)
        analyzer = wrapanalyzer3575.value
        if (not inoutres3576): 
            return None
        ...
        first_pass4301 = True
        while True:
            if first_pass4301: first_pass4301 = False
            else: t = t.next0_
    ...
    def refresh(self, item : 'ExtOntologyItem', definition_ : object) -> bool:
        ...
        analyzer = None
        wrapanalyzer3577 = RefOutArgWrapper(None)
        inoutres3578 = Utils.tryGetValue(self.__m_anal_by_type, item.type_name, wrapanalyzer3577)
        analyzer = wrapanalyzer3577.value
        if (not inoutres3578): 
            return False
    ...
    def _get_analyzer_data(self, type_name : str) -> 'AnalyzerData':
        a = None
        wrapa3579 = RefOutArgWrapper(None)
        inoutres3580 = Utils.tryGetValue(self.__m_anal_by_type, type_name, wrapa3579)
        a = wrapa3579.value
        if (not inoutres3580): 
            return None
    ...
    def __init_hash(self) -> None:
        with self.__m_lock: 
            ...
            for it in self.items: 
                if (it.referent is not None): 
                    ont = None
                    wrapont3582 = RefOutArgWrapper(None)
                    inoutres3583 = Utils.tryGetValue(hash0_, it.referent.type_name, wrapont3582)
                    ont = wrapont3582.value
                    if (not inoutres3583): 
                        ont = IntOntologyCollection._new3581(True)
    ...
    def attach_referent(self, r : 'Referent') -> typing.List['ExtOntologyItem']:
        ...
        if (self.__m_hash is None): 
            self.__init_hash()
        onto = None
        wraponto3584 = RefOutArgWrapper(None)
        inoutres3585 = Utils.tryGetValue(self.__m_hash, r.type_name, wraponto3584)
        onto = wraponto3584.value
        if (not inoutres3585): 
            return None
        li = [ ]
    ...
    def attach_token(self, type_name : str, t : 'Token') -> typing.List['IntOntologyToken']:
        ...
        if (self.__m_hash is None): 
            self.__init_hash()
        onto = None
        wraponto3586 = RefOutArgWrapper(None)
        inoutres3587 = Utils.tryGetValue(self.__m_hash, type_name, wraponto3586)
        onto = wraponto3586.value
        if (not inoutres3587): 
            return None
