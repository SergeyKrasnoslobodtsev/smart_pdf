...
class DateAnalyzer(Analyzer):
        ...
        while True:
            ...
            while i < len(rts): 
                ...
                if ((i < (len(rts) - 1)) and rt.tag is None): 
                    rt.referent.add_occurence(TextAnnotation._new926(kit.sofa, rt.begin_char, rt.end_char, rt.referent))
                else: 
            ...
            if ((dat is not None and t.previous is not None and t.previous.is_hiphen) and t.previous.previous is not None and (isinstance(t.previous.previous.get_referent(), DateReferent))): 
                ...
                dr = Utils.asObjectOrNull(ad.register_referent(DateRangeReferent._new927(dat0, dat)), DateRangeReferent)
            ...
            if ((dat is not None and t.previous is not None and ((t.previous.is_hiphen or t.previous.is_value("ПО", None) or t.previous.is_value("И", None)))) and (isinstance(t.previous.previous, NumberToken)) and t.previous.previous.int_value is not None): 
                ...
                if (dat.day > 0 and (num < dat.day) and num > 0): 
                    if (dat.higher is not None): 
                        dat0 = DateReferent._new928(dat.higher, num)
                    elif (dat.month > 0): 
                        dat0 = DateReferent._new929(dat.month, num)
    ...
    def process_referent_stat(begin : 'Token', param : str=None) -> 'ReferentToken':
        ...
        if (begin.is_value("ДО", None) and (isinstance(begin.next0_, ReferentToken)) and (isinstance(begin.next0_.get_referent(), DateReferent))): 
            drr = DateRangeReferent._new933(Utils.asObjectOrNull(begin.next0_.get_referent(), DateReferent))
        ...
        if (begin.is_value("ПОСЛЕ", None) and (isinstance(begin.next0_, ReferentToken)) and (isinstance(begin.next0_.get_referent(), DateReferent))): 
            drr = DateRangeReferent._new934(Utils.asObjectOrNull(begin.next0_.get_referent(), DateReferent))
            res1 = ReferentToken(drr, begin, begin.next0_)
            if (res1.end_token.next0_ is not None and res1.end_token.next0_.is_value("ВКЛЮЧИТЕЛЬНО", None)): 
                res1.end_token = res1.end_token.next0_
            else: 
    ...
    def try_attach(dts : typing.List['DateItemToken'], high : bool) -> typing.List['ReferentToken']:
            ...
            if (dts[0].typ == DateItemToken.DateItemType.HOUR or ((dts[0].typ == DateItemToken.DateItemType.NUMBER and ((dts[2].typ == DateItemToken.DateItemType.HOUR or dts[2].typ == DateItemToken.DateItemType.NUMBER))))): 
                if (len(dts) > 3 and dts[3].typ == DateItemToken.DateItemType.DELIM and dts[3].string_value == dts[1].string_value): 
                    ...
                    if (res1 is not None and (isinstance(res1[len(res1) - 1].referent, DateReferent)) and res1[len(res1) - 1].referent.day > 0): 
                        time = DateReferent._new935(dts[0].int_value, dts[2].int_value)
        ...
        if (dts[0].typ == DateItemToken.DateItemType.HOUR and len(dts) > 2 and dts[1].typ == DateItemToken.DateItemType.MINUTE): 
            ...
            res1 = DateAnalyzer.try_attach(dts1, False)
            if (res1 is not None and (isinstance(res1[len(res1) - 1].referent, DateReferent)) and res1[len(res1) - 1].referent.day > 0): 
                time = DateReferent._new935(dts[0].int_value, dts[1].int_value)
                if (ii > 2): 
        ...
        year_is_dif = False
        b = False
        wrapyear1000 = RefOutArgWrapper(None)
        wrapmon1001 = RefOutArgWrapper(None)
        wrapday1002 = RefOutArgWrapper(None)
        b = DateAnalyzer.__apply_rule_formal(dts, high, wrapyear1000, wrapmon1001, wrapday1002)
        year = wrapyear1000.value
        mon = wrapmon1001.value
        day = wrapday1002.value
        if (b): 
        ...
        if (len(dts) == 1 and dts[0].ptr == DatePointerType.TODAY): 
            res0 = list()
            res0.append(ReferentToken(DateReferent._new937(DatePointerType.TODAY), dts[0].begin_token, dts[0].end_token))
            return res0
        if (len(dts) == 1 and dts[0].typ == DateItemToken.DateItemType.YEAR and dts[0].year <= 0): 
            res0 = list()
            res0.append(ReferentToken(DateReferent._new937(DatePointerType.UNDEFINED), dts[0].begin_token, dts[0].end_token))
            return res0
        if (not b and dts[0].typ == DateItemToken.DateItemType.POINTER and len(dts) > 1): 
            if (dts[1].typ == DateItemToken.DateItemType.YEAR): 
                ...
                b = True
        if (not b): 
            wrapyear939 = RefOutArgWrapper(None)
            wrapmon940 = RefOutArgWrapper(None)
            wrapday941 = RefOutArgWrapper(None)
            wrapyear_is_dif942 = RefOutArgWrapper(False)
            b = DateAnalyzer.__apply_rule_with_month(dts, high, wrapyear939, wrapmon940, wrapday941, wrapyear_is_dif942)
            year = wrapyear939.value
            mon = wrapmon940.value
            day = wrapday941.value
            year_is_dif = wrapyear_is_dif942.value
        if (not b): 
            wrapyear943 = RefOutArgWrapper(None)
            wrapmon944 = RefOutArgWrapper(None)
            wrapday945 = RefOutArgWrapper(None)
            b = DateAnalyzer.__apply_rule_year_only(dts, wrapyear943, wrapmon944, wrapday945)
            year = wrapyear943.value
            mon = wrapmon944.value
            day = wrapday945.value
        if (not b): 
            ...
            if ((len(dts) == 4 and dts[0].typ == DateItemToken.DateItemType.MONTH and dts[1].typ == DateItemToken.DateItemType.DELIM) and dts[2].typ == DateItemToken.DateItemType.MONTH and dts[3].can_be_year): 
                res0 = list()
                yea = DateReferent._new930(dts[3].int_value)
                res0.append(ReferentToken._new948(yea, dts[3].begin_token, dts[3].end_token, dts[3].morph))
                mon1 = DateReferent._new949(dts[0].int_value, yea)
                res0.append(ReferentToken._new950(mon1, dts[0].begin_token, dts[0].end_token, mon1))
                mon2 = DateReferent._new949(dts[2].int_value, yea)
            ...
            if ((dts[0].typ == DateItemToken.DateItemType.MONTH and len(dts) == 1 and dts[0].end_token.next0_ is not None) and ((dts[0].end_token.next0_.is_hiphen or dts[0].end_token.next0_.is_value("ПО", None) or dts[0].end_token.next0_.is_value("НА", None)))): 
                ...
                if (rt is not None): 
                    ...
                    if ((dr0 is not None and dr0.year > 0 and dr0.month > 0) and dr0.day == 0 and dr0.month > dts[0].int_value): 
                        dr_year0 = DateReferent._new930(dr0.year)
                        res0 = list()
                        res0.append(ReferentToken(dr_year0, dts[0].end_token, dts[0].end_token))
                        dr_mon0 = DateReferent._new949(dts[0].int_value, dr_year0)
            ...
            if (((len(dts) == 3 and dts[1].typ == DateItemToken.DateItemType.DELIM and dts[1].begin_token.is_hiphen) and dts[0].can_be_year and dts[2].can_be_year) and (dts[0].int_value < dts[2].int_value)): 
                ...
                if (ok): 
                    res0 = list()
                    res0.append(ReferentToken(DateReferent._new930(dts[0].year), dts[0].begin_token, dts[0].end_token))
                    res0.append(ReferentToken(DateReferent._new930(dts[2].year), dts[2].begin_token, dts[2].end_token))
                    return res0
            ...
            if (((len(dts) == 5 and dts[0].can_be_day and dts[1].typ == DateItemToken.DateItemType.MONTH) and dts[2].can_be_hour and dts[3].typ == DateItemToken.DateItemType.DELIM) and dts[4].can_be_minute): 
                res0 = list()
                mm = DateReferent._new967(dts[1].int_value)
                res0.append(ReferentToken(mm, dts[1].begin_token, dts[1].end_token))
                dd = DateReferent._new954(dts[0].int_value, mm)
                res0.append(ReferentToken(dd, dts[0].begin_token, dts[2].end_token))
                hh = DateReferent._new969(dts[2].int_value, dts[4].int_value, dd)
                ...
                if (DateAnalyzer.is_user_param_date(dts[0])): 
                    if (((len(dts) == 3 and dts[0].can_be_day and dts[0].typ == DateItemToken.DateItemType.NUMBER) and dts[2].can_by_month and dts[2].typ == DateItemToken.DateItemType.NUMBER) and dts[1].typ == DateItemToken.DateItemType.DELIM and dts[1].begin_token.is_char_of("./\\")): 
                        if (dts[0].begin_token.previous is not None and dts[0].begin_token.previous.is_value("В", None)): 
                            ...
                            res0 = list()
                            y1 = DateReferent._new967(dts[2].int_value)
                            res0.append(ReferentToken(y1, dts[2].begin_token, dts[2].end_token))
                            y2 = DateReferent._new954(dts[0].int_value, y1)
            ...
            if (dts[0].typ == DateItemToken.DateItemType.QUARTAL or dts[0].typ == DateItemToken.DateItemType.HALFYEAR or ((dts[0].typ == DateItemToken.DateItemType.MONTH and dts[0].ltyp != DateItemToken.FirstLastTyp.NO))): 
                if (len(dts) == 1 or dts[1].typ == DateItemToken.DateItemType.YEAR): 
                    ...
                    if (len(dts) > 1): 
                        ii = 1
                        yea = DateReferent._new930(dts[1].int_value)
                        res0.append(ReferentToken._new948(yea, dts[1].begin_token, dts[1].end_token, dts[1].morph))
                    else: 
                        cou = 0
            ...
            if ((len(dts) == 1 and dts[0].typ == DateItemToken.DateItemType.MONTH and dts[0].begin_token.previous is not None) and ((dts[0].begin_token.previous.morph.class0_.is_preposition or dts[0].begin_token.previous.is_comma_and))): 
                if (dts[0].chars.is_latin_letter and dts[0].chars.is_all_lower): 
                    ...
                    res0 = list()
                    res0.append(ReferentToken(DateReferent._new967(dts[0].int_value), dts[0].begin_token, dts[0].end_token))
                    return res0
            if ((len(dts) == 1 and dts[0].typ == DateItemToken.DateItemType.NUMBER and dts[0].can_be_year) and dts[0].length_char == 4): 
                if (dts[0].is_newline_before and dts[0].end_token.next0_ is not None and dts[0].end_token.next0_.is_hiphen): 
                    res0 = list()
                    res0.append(ReferentToken(DateReferent._new930(dts[0].int_value), dts[0].begin_token, dts[0].end_token))
                    return res0
            return None
        res = list()
        dr_year = None
        ...
        t1 = None
        if (cent is not None): 
            ce = DateReferent._new988((- cent.int_value if cent.new_age < 0 else cent.int_value), cent.relate)
            t1 = cent.end_token
            rt = ReferentToken(ce, cent.begin_token, t1)
            res.append(rt)
        if (tenyears is not None): 
            ce = DateReferent._new989(tenyears.int_value, tenyears.relate)
            if (cent is not None): 
        ...
        if (year is not None and year.year > 0): 
            dr_year = DateReferent._new930((- year.year if year.new_age < 0 else year.year))
            if (not year_is_dif): 
                t1 = year.end_token
                if (t1.next0_ is not None and t1.next0_.is_value("ГОРОД", None)): 
                    ...
                    if (tt2 is None): 
                        ...
                        year.end_token = t1
            t0 = year.begin_token
            res.append(ReferentToken._new948(dr_year, t0, year.end_token, year.morph))
            if (((len(dts) == 3 and year == dts[2] and mon is None) and day is None and dts[0].year > 0) and dts[1].typ == DateItemToken.DateItemType.DELIM and dts[1].end_token.is_hiphen): 
                dr_year0 = DateReferent._new930((- dts[0].year if year.new_age < 0 else dts[0].year))
                t0 = dts[0].begin_token
                res.append(ReferentToken(dr_year0, t0, dts[0].end_token))
        if (mon is not None): 
            dr_mon = DateReferent._new967(mon.int_value)
            ...
            if (dr_year is None and t1.next0_ is not None and ((t1.next0_.is_value("ПО", None) or t1.next0_.is_value("НА", None)))): 
                ...
                if (rt is not None): 
                    ...
                    if (dr0 is not None and dr0.year > 0 and dr0.month > 0): 
                        dr_year = DateReferent._new930(dr0.year)
                        t0 = t1
                        res.append(ReferentToken(dr_year, t0, t1))
                        dr_mon.higher = dr_year
            res.append(ReferentToken._new948(dr_mon, t0, t1, mon.morph))
            if (day is not None): 
                dr_day = DateReferent._new996(day.int_value)
                ...
                res.append(ReferentToken._new948(dr_day, t0, t1, day.morph))
                if (dts[0].typ == DateItemToken.DateItemType.HOUR): 
                    hou = DateReferent._new998(dr_day)
            ...
            if (dt0.month == 0): 
                ...
                if (tt is not None and tt.is_char_of("(,")): 
                    ...
                    if (dit is not None and dit.typ == DateItemToken.DateItemType.MONTH): 
                        dr_mon = DateReferent._new999(dt0, dit.int_value)
    ...
    def __try_attach_time_li(dts : typing.List['DateItemToken'], after_date : bool) -> 'ReferentToken':
        ...
        if (len(dts) == 1): 
            if (dts[0].typ == DateItemToken.DateItemType.HOUR and after_date): 
                time = DateReferent._new935(dts[0].int_value, 0)
                t1 = dts[0].end_token
            else: 
                return None
    ...
    def __apply_rule_formal(its : typing.List['DateItemToken'], high : bool, year : 'DateItemToken', mon : 'DateItemToken', day : 'DateItemToken') -> bool:
        ...
        if (high and its[0].can_be_year and len(its) == 1): 
            ...
            if (its[0].new_style is not None and len(its[0].new_style) > 0): 
                year.value = DateItemToken._new1006(its[0].begin_token, its[0].begin_token, its[0].int_value)
            return True
    ...
    def __apply_date_range(ad : 'AnalyzerData', its : typing.List['DateItemToken'], lang : 'MorphLang') -> 'DateRangeReferent':
        ...
        if ((its[0].can_be_year and its[1].string_value == "-" and its[2].typ == DateItemToken.DateItemType.YEAR) and (its[0].year < its[2].year)): 
            ...
            res.date_from = Utils.asObjectOrNull(ad.register_referent(DateReferent._new930(its[0].year)), DateReferent)
            rt1 = ReferentToken(res.date_from, its[0].begin_token, its[0].end_token)
            res.date_to = Utils.asObjectOrNull(ad.register_referent(DateReferent._new930(its[2].year)), DateReferent)
    ...
    def __apply_date_range0(kit : 'AnalysisKit', ad : 'AnalyzerData') -> None:
        ...
        while True:
            ...
            if (str0_ == "ON" and (isinstance(t0, TextToken))): 
                ...
                if (tok is not None): 
                    dow = DateReferent._new1009(tok.termin.tag)
            ...
            if (str0_ == "С" or str0_ == "C"): 
                ...
                if ((isinstance(t.next0_, ReferentToken)) and (isinstance(t.next0_.get_referent(), DateReferent))): 
                    dr = DateRangeReferent._new933(Utils.asObjectOrNull(t.next0_.get_referent(), DateReferent))
                    t00 = t
                ...
                dr = DateRangeReferent._new934(Utils.asObjectOrNull(t.next0_.get_referent(), DateReferent))
            ...
            if (year_val1 == year_val2): 
                ...
                if (DateReferent.compare(date1, date2) >= 0): 
                    continue
            if (date1 is None): 
                date1 = (Utils.asObjectOrNull(ad.register_referent(DateReferent._new930(year_val1)), DateReferent))
                rt0 = ReferentToken(date1, t0, t0)
                kit.embed_token(rt0)
                if (t0 == t): 
                    t = (rt0)
            if (date2 is None): 
                date2 = (Utils.asObjectOrNull(ad.register_referent(DateReferent._new930(year_val2)), DateReferent))
                rt1 = ReferentToken(date2, t1, t1)
                kit.embed_token(rt1)
                t1 = (rt1)
            rt = ReferentToken(ad.register_referent(DateRangeReferent._new927(date1, date2)), t, t1)
