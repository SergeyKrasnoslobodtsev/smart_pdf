...
class ReferentClass:
    ...
    def add_feature(self, attr_name : str, attr_caption : str, low_bound : int=0, up_bound : int=0) -> 'Feature':
        ...
        """
        res = Feature._new2380(attr_name, attr_caption, low_bound, up_bound)
    ...
    def find_feature(self, name_ : str) -> 'Feature':
        ...
        """
        ind = 0
        wrapind2381 = RefOutArgWrapper(0)
        inoutres2382 = Utils.tryGetValue(self.__m_hash, name_, wrapind2381)
        ind = wrapind2381.value
        if (not inoutres2382): 
            return None
        else: 
            return self.__m_features[ind]
