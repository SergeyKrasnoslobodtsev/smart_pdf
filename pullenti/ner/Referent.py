...
class Referent:
    ...
    def get_int_value(self, attr_name : str, def_value : int) -> int:
        ...
        if (Utils.isNullOrEmpty(str0_)): 
            return def_value
        res = 0
        wrapres3616 = RefOutArgWrapper(0)
        inoutres3617 = Utils.tryParseInt(str0_, wrapres3616)
        res = wrapres3616.value
        if (not inoutres3617): 
            return def_value
        return res
    ...
    def add_occurence_of_ref_tok(self, rt : 'ReferentToken') -> None:
        self.add_occurence(TextAnnotation._new926(rt.kit.sofa, rt.begin_char, rt.end_char, rt.referent))
    ...
    def add_occurence(self, anno : 'TextAnnotation') -> None:
        ...
        if (anno.occurence_of != self and anno.occurence_of is not None): 
            anno = TextAnnotation._new3619(anno.begin_char, anno.end_char, anno.sofa)
        if (self.__m_occurrence is None): 
            self.__m_occurrence = list()
    ...
    def clone(self) -> 'Referent':
        ...
        for r in self.slots: 
            rr = Slot._new3620(r.type_name, r.value, r.count)
            rr.owner = res
            res.slots.append(rr)
        return res
    ...
    def deserialize(self, stream : Stream, all0_ : typing.List['Referent'], sofa : 'SourceOfAnalysis', new_format : bool=False) -> None:
        ...
        i = 0
        while i < cou: 
            a = TextAnnotation._new3621(sofa, self)
