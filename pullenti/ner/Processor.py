...
class Processor(object):
    ...
    def find_analyzer(self, name : str) -> 'Analyzer':
        ...
        a = None
        with self.__m_lock: 
            wrapa3603 = RefOutArgWrapper(None)
            inoutres3604 = Utils.tryGetValue(self.__m_analyzers_hash, Utils.ifNotNull(name, ""), wrapa3603)
            a = wrapa3603.value
            if (inoutres3604): 
                return a
    ...
    def process_next(self, ar : 'AnalysisResult') -> None:
        ...
        if (ar is None): 
            return
        kit = AnalysisKit._new3605(self, ar.ontology)
        kit._init_from(ar)
    ...
    def _process(self, text : 'SourceOfAnalysis', onto_regine : bool, no_log : bool, ext_ontology : 'ExtOntology'=None, lang : 'MorphLang'=None) -> 'AnalysisResult':
        ...
        kit = AnalysisKit._new3606(text, False, lang, self.__progress_changed_event_handler_on_progress_handler, ext_ontology, self, onto_regine)
        ar = AnalysisResult()
    ...
    def __process2(self, kit : 'AnalysisKit', ar : 'AnalysisResult', no_log : bool) -> None:
        ...
        first_pass4303 = True
        while True:
            if first_pass4303: first_pass4303 = False
            else: ii += 1
    ...
    def __prepare_progress(self) -> None:
        with self.__m_progress_peaces_lock: 
            ...
            max0_ = co * 100
            max0_ /= (total)
            self.__m_progress_peaces[self] = ProgressPeace._new3607(0, max0_)
            for wf in self.analyzers: 
                if (not wf.ignore_this_analyzer): 
                    min0_ = max0_
                    ...
                    if (not wf in self.__m_progress_peaces): 
                        self.__m_progress_peaces[wf] = ProgressPeace._new3607(min0_, max0_)
    ...
    def _on_progress_handler(self, sender : object, e0_ : ProgressEventArgs) -> None:
        ...
        if (e0_.progressPercentage >= 0): 
            pi0_ = None
            with self.__m_progress_peaces_lock: 
                wrappi3609 = RefOutArgWrapper(None)
                inoutres3610 = Utils.tryGetValue(self.__m_progress_peaces, Utils.ifNotNull(sender, self), wrappi3609)
                pi0_ = wrappi3609.value
                if (inoutres3610): 
