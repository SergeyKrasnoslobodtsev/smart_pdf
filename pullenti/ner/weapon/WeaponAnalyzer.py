...
class WeaponAnalyzer(Analyzer):
    ...
    def process(self, kit : 'AnalysisKit') -> None:
        ...
        first_pass4297 = True
        while True:
            if first_pass4297: first_pass4297 = False
            else: t = t.next0_
                        ...
                        if (s.type_name == WeaponReferent.ATTR_MODEL): 
                            ...
                            for k in range(2):
                                if (not str.isdigit(mod[0])): 
                                    li = [ ]
                                    wrapli3570 = RefOutArgWrapper(None)
                                    inoutres3571 = Utils.tryGetValue(objs_by_model, mod, wrapli3570)
                                    li = wrapli3570.value
                                    if (not inoutres3571): 
                                        li = list()
                            ...
                            obj_by_names.add(Termin._new84(str(s.value), rt.referent))
        if (len(objs_by_model) == 0 and len(obj_by_names.termins) == 0): 
            return
        t = kit.first_token
        first_pass4298 = True
        while True:
            if first_pass4298: first_pass4298 = False
            else: t = t.next0_
            if (not (t is not None)): break
            if (t.is_ignored): 
                continue
    ...
    def __try_attach(self, its : typing.List['WeaponItemToken'], attach : bool) -> typing.List['ReferentToken']:
        ...
        first_pass4299 = True
        while True:
            if first_pass4299: first_pass4299 = False
            else: i += 1
        ...
        if (noun is None and model is not None): 
            ...
            first_pass4300 = True
            while True:
                if first_pass4300: first_pass4300 = False
