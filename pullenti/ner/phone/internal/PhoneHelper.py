...
class PhoneHelper:
    ...
    def initialize() -> None:
        ...
        for line0 in Utils.splitString(str0_, '\n', False): 
            ...
            while i < len(cod): 
                dig = cod[i]
                nn = None
                wrapnn3368 = RefOutArgWrapper(None)
                inoutres3369 = Utils.tryGetValue(tn.children, dig, wrapnn3368)
                nn = wrapnn3368.value
                if (not inoutres3369): 
    ...
    def get_country_prefix(full_number : str) -> str:
        ...
        while i < len(full_number): 
            dig = full_number[i]
            nn = None
            wrapnn3370 = RefOutArgWrapper(None)
            inoutres3371 = Utils.tryGetValue(nod.children, dig, wrapnn3370)
            nn = wrapnn3370.value
            if (not inoutres3371): 
                break
