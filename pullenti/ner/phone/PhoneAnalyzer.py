...
class PhoneAnalyzer(Analyzer):
    ...
    class PhoneAnalizerData(AnalyzerData):
        ...
        def register_referent(self, referent : 'Referent') -> 'Referent':
            ...
            if (len(key) >= 10): 
                key = key[3:]
            ph_li = [ ]
            wrapph_li3399 = RefOutArgWrapper(None)
            inoutres3400 = Utils.tryGetValue(self.__m_phones_hash, key, wrapph_li3399)
            ph_li = wrapph_li3399.value
            if (not inoutres3400): 
                ph_li = list()
    ...
    def process(self, kit : 'AnalysisKit') -> None:
        ...
        first_pass4245 = True
        while True:
            if first_pass4245: first_pass4245 = False
            else: t = t.next0_
