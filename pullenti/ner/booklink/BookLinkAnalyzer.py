...
class BookLinkAnalyzer(Analyzer):
    ...
    def process(self, kit : 'AnalysisKit') -> None:
        ...
        while True:
            ...
            if (re.number is not None): 
                li = [ ]
                wrapli515 = RefOutArgWrapper(None)
                inoutres516 = Utils.tryGetValue(refs_by_num, re.number, wrapli515)
                li = wrapli515.value
                if (not inoutres516): 
                    li = list()
        ...
        while True:
            ...
            li = [ ]
            wrapli517 = RefOutArgWrapper(None)
            inoutres518 = Utils.tryGetValue(refs_by_num, Utils.ifNotNull(re.number, ""), wrapli517)
            li = wrapli517.value
            if (not inoutres518): 
                continue
            i = 0
    ...
    def __try_parse(t : 'Token', is_in_lit : bool, max_char : int=0) -> typing.List['ReferentToken']:
        ...
        next_num = None
        nn = 0
        wrapnn523 = RefOutArgWrapper(0)
        inoutres524 = Utils.tryParseInt(Utils.ifNotNull(num, ""), wrapnn523)
        nn = wrapnn523.value
        if (inoutres524): 
            next_num = str((nn + 1))
        ...
        if (tn1 is None and start_of_name is None): 
            if (is_electr_res): 
                ...
                rts0 = list()
                bref0 = BookLinkRefReferent._new519(uri_re)
                if (num is not None): 
                    bref0.number = num
            ...
            if (decree is not None and num is not None): 
                rts0 = list()
                bref0 = BookLinkRefReferent._new519(decree.get_referent())
                if (num is not None): 
                    bref0.number = num
            ...
            if (book_prev is not None): 
                ...
                if (blt0 is not None and blt0.typ == BookLinkTyp.PAGERANGE): 
                    rts0 = list()
                    bref0 = BookLinkRefReferent._new519(book_prev)
                    if (num is not None): 
                        bref0.number = num
        ...
        rts = list()
        bref = BookLinkRefReferent._new519(res)
        if (num is not None): 
            bref.number = num
