...
class BookLinkReferent(Referent):
    ...
    def year(self) -> int:
        """ Год """
        year_ = 0
        wrapyear525 = RefOutArgWrapper(0)
        inoutres526 = Utils.tryParseInt(Utils.ifNotNull(self.get_string_value(BookLinkReferent.ATTR_YEAR), ""), wrapyear525)
        year_ = wrapyear525.value
        if (inoutres526): 
            return year_
        else: 
            return 0
