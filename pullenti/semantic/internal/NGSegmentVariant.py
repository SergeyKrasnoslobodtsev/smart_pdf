...
class NGSegmentVariant(object):
    ...
    def calc_coef(self) -> float:
        ...
        first_pass4314 = True
        while True:
            if first_pass4314: first_pass4314 = False
            else: i += 1
            ...
            first_pass4315 = True
            while True:
                if first_pass4315: first_pass4315 = False
                else: k += 1
                if (not (k < i)): break
                li = self.links[k]
                if (li is None): 
        ...
        first_pass4316 = True
        while True:
            if first_pass4316: first_pass4316 = False
            else: i += 1
            if (not (i < len(self.links))): break
            ...
            if (k >= (len(list0_) - 1) and list0_[k].and_before): 
                ...
                if (k >= len(list0_)): 
                    ...
                    self.coef = -1
                    return self.coef
            ngli = NGLink._new3689(NGLinkType.LIST)
            k = 0
            ...
            if (not ok or lalink is None): 
                continue
            test = NGLink._new3690(lalink.from0_, lalink.typ)
            j = 0
            j = 0
        ...
        first_pass4317 = True
        while True:
            if first_pass4317: first_pass4317 = False
            else: i += 1
            if (not (i < len(self.links))): break
            li = self.links[i]
            if (li is None): 
        ...
        first_pass4318 = True
        while True:
            if first_pass4318: first_pass4318 = False
            else: i += 1
            if (not (i < len(self.links))): break
            li = self.links[i]
            if (li is None): 
        ...
        first_pass4319 = True
        while True:
            if first_pass4319: first_pass4319 = False
            else: i += 1
            if (not (i < len(self.links))): break
            li = self.links[i]
            if (li is None): 
    ...
    def get_list(self, ord0_ : int) -> typing.List['NGItem']:
        ...
        if (li.typ == NGLinkType.LIST): 
            if (li.to_verb is None): 
                return None
            res = list()
            res.append(NGItem._new3691(SentItem(li.to_verb), ord0_ - 1))
            res.append(ngit)
        i = ord0_ + 1
        first_pass4320 = True
        while True:
            if first_pass4320: first_pass4320 = False
            else: i += 1
            if (not (i < len(self.links))): break
            li = self.links[i]
    ...
    def correct_morph(self) -> None:
        ...
        first_pass4321 = True
        while True:
            if first_pass4321: first_pass4321 = False
            else: i += 1
            if (not (i < len(self.links))): break
            li = self.links[i]
            if (li is None): 
    ...
    @staticmethod
    def _new3688(_arg1 : 'NGSegment') -> 'NGSegmentVariant':
