...
class SentItem:
    ...
    def add_attr(self, adv : 'AdverbToken') -> None:
        sa = SemAttribute._new3703(adv.spelling, adv.typ, adv.not0_)
        if (self.attrs is None): 
            self.attrs = list()
        self.attrs.append(SemAttributeEx._new3704(adv, sa))
            ...
            if (len(num.units) == 0): 
                ...
                if ((npt1 is not None and npt1.end_token.is_value("ОНИ", None) and npt1.preposition is not None) and npt1.preposition.normal == "ИЗ"): 
                    ...
                    if (nn == "1" and (isinstance(num.end_token, NumberToken)) and num.end_token.end_token.is_value("ОДИН", None)): 
                        a = SemAttribute._new3705(SemAttributeType.ONEOF, num.end_token.end_token.get_normal_case_text(None, MorphNumber.SINGULAR, MorphGender.UNDEFINED, False))
                        aex = SemAttributeEx._new3704(num, a)
                        si1.attrs = list()
                    ...
                    return res
                if (npt1 is not None): 
                    si1 = SentItem._new3707(npt1, SemQuantity(str(num), num.begin_token, num.end_token))
                    if (prep_ is not None): 
                        si1.prep = prep_.normal
        ...
        if (adv is not None): 
            if (adv.typ == SemAttributeType.OTHER): 
                ...
                if (npt1 is not None and npt1.end_token.is_value("ОНИ", None) and npt1.preposition is not None): 
                    si1 = SentItem(npt1)
                    a = SemAttribute._new3705(SemAttributeType.OTHER, adv.end_token.get_normal_case_text(None, MorphNumber.UNDEFINED, MorphGender.UNDEFINED, False))
                    aex = SemAttributeEx._new3704(num, a)
                    si1.attrs = list()
                    si1.attrs.append(aex)
                    if (prep_ is not None): 
                    ...
                    if (prev[i].attrs is not None): 
                        for a in prev[i].attrs: 
                            if (a.attr.typ == SemAttributeType.ONEOF): 
                                ...
                                aa = SemAttribute._new3705(SemAttributeType.OTHER, adv.end_token.get_normal_case_text(None, MorphNumber.UNDEFINED, MorphGender.UNDEFINED, False))
                                aex = SemAttributeEx._new3704(adv, aa)
                                si1.attrs = list()
                                si1.attrs.append(aex)
                                if (prep_ is not None): 
                                    si1.prep = prep_.normal
            ...
            return res
        if (mc.is_adjective): 
            npt = NounPhraseToken._new3712(t, t, MorphCollection(t.morph))
    ...
    def __parse_participles(vb : 'VerbPhraseToken', t1 : 'Token', lev : int) -> typing.List['SentItem']:
        ...
        for s in sents: 
            ...
            first_pass4338 = True
            while True:
                if first_pass4338: first_pass4338 = False
                else: i += 1
                ...
                if (it.typ == SentItemType.PARTBEFORE or it.typ == SentItemType.PARTAFTER): 
                    continue
                li = NGLink._new3714(typ_, NGItem._new3685(it), vb)
                li.calc_coef(True)
                if (li.coef < 0): 
                    continue
                if (it.end_token.end_char in endpos): 
                    continue
                ss = Sentence._new3715(typ_)
    ...
    @staticmethod
    def _new3707(_arg1 : 'MetaToken', _arg2 : 'SemQuantity') -> 'SentItem':
        res = SentItem(_arg1)
        res.quant = _arg2
        return res