...
class AdverbToken(MetaToken):
        ...
        if (t.is_value("ДРУГ", None) or t.is_value("САМ", None)): 
            ...
            if (t1 is not None): 
                if (t1.is_value("ДРУГ", None) and t.is_value("ДРУГ", None)): 
                    return AdverbToken._new3647(t0, t1, SemAttributeType.EACHOTHER)
                if (t1.is_value("СЕБЯ", None) and t.is_value("САМ", None)): 
                    return AdverbToken._new3647(t0, t1, SemAttributeType.HIMELF)
        tok = AdverbToken.__m_termins.try_parse(t, TerminParseAttr.NO)
        if (tok is not None): 
            res = AdverbToken._new3647(t0, tok.end_token, Utils.valToEnum(tok.termin.tag, SemAttributeType))
    ...
    def initialize() -> None:
        ...
        t = None
        t = Termin._new84("ЕЩЕ", SemAttributeType.STILL)
        AdverbToken.__m_termins.add(t)
        t = Termin._new84("УЖЕ", SemAttributeType.ALREADY)
        AdverbToken.__m_termins.add(t)
        t = Termin._new84("ВСЕ", SemAttributeType.ALL)
        AdverbToken.__m_termins.add(t)
        t = Termin._new84("ЛЮБОЙ", SemAttributeType.ANY)
        t.add_variant("ЛЮБОЙ", False)
        t.add_variant("КАЖДЫЙ", False)
        t.add_variant("ЧТО УГОДНО", False)
        t.add_variant("ВСЯКИЙ", False)
        AdverbToken.__m_termins.add(t)
        t = Termin._new84("НЕКОТОРЫЙ", SemAttributeType.SOME)
        t.add_variant("НЕКИЙ", False)
        AdverbToken.__m_termins.add(t)
        t = Termin._new84("ДРУГОЙ", SemAttributeType.OTHER)
        t.add_variant("ИНОЙ", False)
        AdverbToken.__m_termins.add(t)
        t = Termin._new84("ВЕСЬ", SemAttributeType.WHOLE)
        t.add_variant("ЦЕЛИКОМ", False)
        t.add_variant("ПОЛНОСТЬЮ", False)
        AdverbToken.__m_termins.add(t)
        t = Termin._new84("ОЧЕНЬ", SemAttributeType.VERY)
        AdverbToken.__m_termins.add(t)
        t = Termin._new84("МЕНЬШЕ", SemAttributeType.LESS)
        t.add_variant("МЕНЕЕ", False)
        t.add_variant("МЕНЕЕ", False)
        t.add_variant("МЕНЬШЕ", False)
        AdverbToken.__m_termins.add(t)
        t = Termin._new84("БОЛЬШЕ", SemAttributeType.GREAT)
        t.add_variant("БОЛЕЕ", False)
        t.add_variant("СВЫШЕ", False)
        AdverbToken.__m_termins.add(t)
    
    @staticmethod
    def _new3647(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'SemAttributeType') -> 'AdverbToken':
