...
class DerivateDictionary:
    ...
    def __load_tree_node(self, tn : 'ExplanTreeNode') -> None:
        with self._m_lock: 
            pos = tn.lazy_pos
            if (pos > 0): 
                wrappos3680 = RefOutArgWrapper(pos)
                tn._deserialize(self.__m_buf, self, True, wrappos3680)
                pos = wrappos3680.value
            tn.lazy_pos = 0
    ...
    def _deserialize(self, str0_ : Stream, lazy_load : bool) -> None:
        ...
        with MemoryStream() as tmp: 
            ...
            pos = 0
            wrappos3684 = RefOutArgWrapper(pos)
            cou = wr.deserialize_int(wrappos3684)
            pos = wrappos3684.value
            while cou > 0: 
                wrappos3682 = RefOutArgWrapper(pos)
                p1 = wr.deserialize_int(wrappos3682)
                pos = wrappos3682.value
                ew = DerivateGroup()
                if (lazy_load): 
                    ew._lazy_pos = pos
                    pos = p1
                else: 
                    wrappos3681 = RefOutArgWrapper(pos)
                    ew._deserialize(wr, wrappos3681)
                    pos = wrappos3681.value
                ew.id0_ = (len(self._m_all_groups) + 1)
                self._m_all_groups.append(ew)
                cou -= 1
            self._m_root = ExplanTreeNode()
            wrappos3683 = RefOutArgWrapper(pos)
            self._m_root._deserialize(wr, self, lazy_load, wrappos3683)
            pos = wrappos3683.value
        self.__m_buf = wr
