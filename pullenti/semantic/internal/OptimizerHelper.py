...
class OptimizerHelper:
    ...
    def __process_participles(gr : 'SemGraph') -> bool:
        ...
        first_pass4322 = True
        while True:
            if first_pass4322: first_pass4322 = False
            else: i += 1
            ...
            if (not has): 
                continue
            if (own is None): 
                dum = SemObject._new3692(gr, SemObjectType.NOUN)
                if (obj.morph is not None): 
                    dum.morph = MorphWordForm._new3693(MorphClass.NOUN, obj.morph.number, obj.morph.gender, obj.morph.case_)
    ...
    def __collapse_anafors(gr : 'SemGraph') -> bool:
        ...
        first_pass4323 = True
        while True:
            if first_pass4323: first_pass4323 = False
            else: i += 1
            if (not (i < len(gr.objects))): break
            obj = gr.objects[i]
    ...
    def __process_formulas(objs : typing.List['SemObject']) -> bool:
        ...
        first_pass4324 = True
        while True:
            if first_pass4324: first_pass4324 = False
            else: i += 1
    ...
    def __process_pointers(objs : typing.List['SemObject']) -> bool:
        ...
        first_pass4325 = True
        while True:
            if first_pass4325: first_pass4325 = False
            else: i += 1
            if (not (i < len(objs))): break
            o = objs[i]
            ...
            if (not ok): 
                ...
                first_pass4326 = True
                while True:
                    if first_pass4326: first_pass4326 = False
                    else: j += 1
            ...
            if (not ok): 
                continue
            first = SemObject._new3692(o.graph, SemObjectType.ADJECTIVE)
        ...
        first_pass4327 = True
        while True:
            if first_pass4327: first_pass4327 = False
            else: i += 1
            if (not (i < len(objs))): break
            o = objs[i]
            if (o.typ != SemObjectType.NOUN): 
