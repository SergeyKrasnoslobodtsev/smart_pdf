...
class NGSegment:
    ...
    def create_segments(s : 'Sentence') -> typing.List['NGSegment']:
        ...
        first_pass4310 = True
        while True:
            if first_pass4310: first_pass4310 = False
            else: i += 1
            ...
            first_pass4311 = True
            while True:
                if first_pass4311: first_pass4311 = False
                else: i += 1
                if (not (i < len(s.items))): break
                it = s.items[i]
                if (it.can_be_comma_end): 
                ...
                if (it.can_be_noun or it.typ == SentItemType.ADVERB): 
                    nit = NGItem._new3686(it, comma, and0_, or0_)
    ...
    def create_links(self, after_part : bool=False) -> None:
        ...
        first_pass4312 = True
        while True:
            if first_pass4312: first_pass4312 = False
            else: i += 1
        ...
        first_pass4313 = True
        while True:
            if first_pass4313: first_pass4313 = False
            else: i += 1
            if (not (i < len(self.items))): break
            it = self.items[i]
            ...
            if (len(it0.links) > 0): 
                continue
            li = NGLink._new3687(NGLinkType.GENETIVE, it0, it, True)
            li.calc_coef(True)
            if (li.coef > 0): 
                it0.links.append(li)
    ...
    def create_variants(self, max_count : int=5) -> None:
        ...
        for kkk in range(1000):
            if (var is None): 
                var = NGSegmentVariant._new3688(self)
            else: 
                var.links.clear()
            i = 0
            while i < len(self.items): 
