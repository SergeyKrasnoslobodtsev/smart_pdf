...
class Sentence(object):
    ...
    def __create_lists(self, s : 'NGSegmentVariant') -> None:
        ...
        first_pass4328 = True
        while True:
            if first_pass4328: first_pass4328 = False
            else: i += 1
    ...
    def __create_links(self, s : 'NGSegmentVariant') -> None:
        ...
        first_pass4329 = True
        while True:
            if first_pass4329: first_pass4329 = False
            else: i += 1
            if (not (i < len(s.links))): break
            link0 = s.links[i]
            if (link0 is None): 
                ...
                if (li.to is not None and li.to.res_object is not None): 
                    ...
                    if (li.typ == NGLinkType.BE): 
                        if ((isinstance(li.from0_.source.source, NumbersWithUnitToken)) and li.to is not None): 
                            ...
                            continue
                        be = SemObject._new3692(gr, SemObjectType.VERB)
    ...
    def __create_result(self, blk : 'SemBlock') -> None:
                ...
                if (isinstance(it.source, NounPhraseToken)): 
                    ...
                    if (npt.multi_nouns and it.result.quantity is None): 
                        ...
                        while i < len(npt.adjectives): 
                            so = SemObject._new3692(it.res_graph, it.result.typ)
        ...
        for it in self.items: 
            if (it.result is not None and it.typ == SentItemType.VERB and (isinstance(it.source, VerbPhraseToken))): 
                if (agent is not None): 
                    ...
                    for li in it.res_graph.links: 
                        if (li.typ == SemLinkType.PACIENT and li.source == it.result): 
                            ...
                            break
                    if (not has_pac): 
                        ni0 = NGItem._new3685(agent)
                        gli0 = NGLink._new3698(ni0, Utils.asObjectOrNull(it.source, VerbPhraseToken), NGLinkType.PACIENT)
                ...
                if (agent is None): 
                    continue
                ni = NGItem._new3685(agent)
                gli = NGLink._new3698(ni, Utils.asObjectOrNull(it.source, VerbPhraseToken), NGLinkType.AGENT)
                if (agent.result_list is not None): 
        ...
        first_pass4331 = True
        while True:
            if first_pass4331: first_pass4331 = False
            else: i += 1
            if (not (i < len(self.items))): break
            it = self.items[i]
            ...
            if (link is None): 
                ...
                first_pass4332 = True
                while True:
                    if first_pass4332: first_pass4332 = False
                    else: j += 1
    ...
    def parse_variants(t0 : 'Token', t1 : 'Token', lev : int, max_count : int=0, regime : 'SentItemType'=SentItemType.UNDEFINED) -> typing.List['Sentence']:
        ...
        first_pass4333 = True
        while True:
            if first_pass4333: first_pass4333 = False
            else: t = t.next0_
            ...
            for it in items_: 
                nexts = None
                wrapnexts3701 = RefOutArgWrapper(None)
                inoutres3702 = Utils.tryGetValue(m_nexts, it.end_token.end_char, wrapnexts3701)
                nexts = wrapnexts3701.value
                if (not inoutres3702): 
    ...
    def calc_coef(self, no_result : bool) -> None:
        ...
        first_pass4334 = True
        while True:
            if first_pass4334: first_pass4334 = False
            else: i += 1
            if (not (i < len(self.items))): break
            it = self.items[i]
    ...
    @staticmethod
    def _new3715(_arg1 : 'NGLinkType') -> 'Sentence':
        res = Sentence()
